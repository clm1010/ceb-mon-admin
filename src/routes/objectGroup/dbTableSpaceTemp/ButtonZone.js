...
function buttonZone({
	...
	const onAdd = () => {
		dispatch({
			type: 'dbTableSpaceTemp/appcategories',
			payload: {
				q: 'affectSystem=="网络|*"'
			}
		})
		dispatch({
			type: 'dbTableSpaceTemp/setState',				//@@@
			payload: {
				modalType: 'create',
				currentItem: {},
				modalVisible: true,
	...
	const onDelete = () => {
		confirm({
			...
			onOk() {
				...
				dispatch({
					type: 'dbTableSpaceTemp/deleteAll',				//@@@
					payload: ids,
				})
			},
		})
	...
	const onRulePreview = () => {
		...
		window.open(`/rulespreview?ids=${ids}&branches=${branches}&firstClass=db`, '批量下发预览', '', 'false')
	}
