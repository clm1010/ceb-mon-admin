...
//@@@
const dbTableSpaceTemp = ({
	location, dispatch, dbTableSpaceTemp, moSingleSelect, loading, appSelect
}) => {
	const {
		q,
		list,
		...
		thirdClass,
		appCategorlist,
	} = dbTableSpaceTemp					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
	...
	const listProps = { //这里定义的是表格对应的数据源与配置
		dispatch,
		dataSource: list,
		loading: loading.effects['dbTableSpaceTemp/query'],
		pagination,
		secondClass,
		q,
		batchDelete,
...
export default connect(({ dbTableSpaceTemp, moSingleSelect, loading, appSelect }) => ({ dbTableSpaceTemp, moSingleSelect, loading, appSelect }))(dbTableSpaceTemp)	//@@@
