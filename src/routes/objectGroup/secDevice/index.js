...
//@@@
const secDevice = ({
 app, location, dispatch, secDevice, policyList, interfaceList, interfacer, moSingleSelect, loading, appSelect,
}) => {
	const {
		q,
		list,
		...
		pageChange,
		appCategorlist,
	} = secDevice					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['secDevice/query'],
    pagination,
    location,
    key: pageChange,
    q,
...
export default connect(({
 app, secDevice, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}) => ({
 app, secDevice, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}))(secDevice)	//@@@
