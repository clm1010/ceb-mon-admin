...
const modal = ({
	...
	const onCancel = () => { //弹出窗口中点击取消按钮触发的函数
		resetFields()
		dispatch({
			type: 'switcher/setState',				//@@@//抛一个事件给监听这个type的监听器
			payload: {
				modalVisible: false,
				FScloud:false,
				snmpVerflag:true,
			...
			<Form layout="horizontal">
				...
				<span style={{ width: '33%', float: 'left' }}>
					<FormItem label="一级安全域" key="firstSecArea" hasFeedback {...formItemLayout}>
						...
						})(<Select onSelect={onChangeFirstSecArea} disabled={modalType === 'update' && !(user.name === 'admin' || user.roles[0].name === '超级管理员')}>
