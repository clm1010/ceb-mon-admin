...
//@@@
const switcher = ({
 app, location, dispatch, switcher, policyList, interfaceList, interfacer, moSingleSelect, loading, appSelect,
}) => {
	const {
		q,
		list,
		...
		snmpVerflag,
		optionSelectAppName,
	} = switcher					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['switcher/query'],
    pagination,
    location,
    key: pageChange,
    q,
...
export default connect(({
 app, switcher, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}) => ({
 app, switcher, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}))(switcher)	//@@@
