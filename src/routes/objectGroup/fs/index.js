...
//@@@
const fs = ({
 location, dispatch, fs, loading, app, appSelect,osSingleSelect,
}) => {
	const {
		q,
		list,
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['fs/query'],
    pagination,
    q,
    key: pageChange,
    batchDelete,
...
export default connect(({
 fs, appSelect, loading, app,osSingleSelect
}) => ({
 fs, appSelect, app, loading,osSingleSelect
}))(fs) //@@@
