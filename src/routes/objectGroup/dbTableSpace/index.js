...
//@@@
const dbTableSpace = ({
	location, dispatch, dbTableSpace, moSingleSelect, loading,appSelect
   }) => {
	const {
		q,
		list,
		...
		thirdClass,
		appCategorlist,
	} = dbTableSpace					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
	...
	q: `firstClass=='DB';secondClass==${secondClass};thirdClass=='DB_TABLE_SPACE'`,
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['dbTableSpace/query'],
    pagination,
    secondClass,
    q,
    batchDelete,
...
export default connect(({ dbTableSpace, moSingleSelect, loading, appSelect }) => ({ dbTableSpace, moSingleSelect, loading,appSelect }))(dbTableSpace)	//@@@
