...
//@@@
const ssl = ({
 app, location, dispatch, ssl, policyList, interfaceList, interfacer, moSingleSelect, loading, appSelect,
}) => {
	const {
		q,
		list,
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['ssl/query'],
    pagination,
    location,
    key: pageChange,
    q,
...
export default connect(({
 app, ssl, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}) => ({
 app, ssl, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}))(ssl)	//@@@
