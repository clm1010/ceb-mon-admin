...
//@@@
const middleware = ({
 location, dispatch, middleware, appSelect, loading, app, policyList
}) => {
	const {
		q,
		list,
		...
		FScloud,
		optionSelectAppName,
	} = middleware					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
...
const policyListProps = {
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['middleware/query'],
    pagination,
    key: pageChange,
    q,
    batchDelete,
...
export default connect(({
 middleware, appSelect, app, loading, policyList
}) => ({
 middleware, appSelect, app, loading, policyList
}))(middleware) //@@@
