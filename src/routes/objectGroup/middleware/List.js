...
function list({
	...
	const onPreview = (record) => {
		window.open(`/rulespreview?ids=${record.mo.uuid}&branches=${record.mo.branchName}&firstClass=mw`, `${record.mo.uuid}`, '', 'false')
	}
	...
	const onEdit = (record) => {
		dispatch({
			type: 'middleware/appcategories',
			payload: {
				q: 'affectSystem=="网络|*"'
			}
		})
		dispatch({
			type: 'middleware/findById',				//@@@
			payload: {
