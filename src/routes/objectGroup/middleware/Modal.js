...
import { Form, Input, Modal, Select, Alert, message, InputNumber, Divider } from 'antd'
...
const modal = ({
	...
	const onMngtOrg = (value, record) => {
		item.mngtOrg = record.props.children
		item.mngtOrgCode = value
		dispatch({
			type: 'middleware/setState',				//@@@
			payload: {
				currentItem: item,
			},
		})
	...
	const onOk = () => {
		validateFieldsAndScroll((errors) => {
			...
			if (appSelect.currentItem.c1 && modalType === 'create') {
				dispatch({
					type: `middleware/${modalType}`,				//@@@
					payload: {
						currentItem: data,
					},
				})
				dispatch({
					type: 'middleware/setState',				//@@@//抛一个事件给监听这个type的监听器
					payload: {
						modalVisible: false,
					},
				})
	...
	//假装验证
	const fakeBlurFunctions = (rule, value, callback) => {
		callback()
	}
	// 定义secondClass所带来的区别
	let secondClass = location.query ? location.query.secondClass : item.secondClass;
	let secondClassDisabled = secondClass ? true : false;
	let jmxdisplay = 'none'
	let jmxrequire = false
	let jmxIPvalidator = fakeBlurFunctions
	if (secondClass && (secondClass === 'MW_WEBLOGIC' || secondClass === 'MW_TOMCAT')) {
		jmxdisplay = ''
		jmxrequire = true
		jmxIPvalidator = blurFunctions
	}
	const deciFScloud = (e) => {
		dispatch({
			type: 'middleware/setState',
	...
	//end
	return (
		<Modal {...modalOpts} width={1050} key="dataBaseModal">
			<Form layout="horizontal">
				...
				<span style={{ width: '33%', float: 'left' }}>
					<FormItem label="类型" key="secondClass" hasFeedback {...formItemLayout}>
						...
						})(<Select disabled={secondClassDisabled}>
							<Select.Option value="MW_WEBLOGIC">Weblogic</Select.Option>
							<Select.Option value="MW_NGINX">Nginx</Select.Option>
							<Select.Option value="MW_APACHE">Apache</Select.Option>
							<Select.Option value="MW_TOMCAT">Tomcat</Select.Option>
							<Select.Option value="MW_WEBSPHERE">Websphere</Select.Option>
							<Select.Option value="MW_KAFKA">Kafka</Select.Option>
						</Select>)}
					</FormItem>
				</span>
				<span style={{ width: '33%', float: 'left' }}>
				...
				<span style={{ width: '33%', float: 'left', display: jmxdisplay }}>
					<FormItem label="jmx IP" key="jmxIP" hasFeedback {...formItemLayout}>
						{getFieldDecorator('jmxIP', {
							initialValue: item.jmxIP,
							rules: [
								{
									required: jmxrequire, message: 'jmxIP不能为空',
								},
								{
									validator: validateIP,
								},
				...
				<span style={{ width: '33%', float: 'left', display: jmxdisplay }} >
					<FormItem label="jmx 端口" key="jmxPort" hasFeedback {...formItemLayout}>
						{getFieldDecorator('jmxPort', {
							initialValue: item.jmxPort,
							rules: [
								{
									required: jmxrequire,
								},
								{
									validator: jmxIPvalidator,
								}
				...
				<span style={{ width: '33%', float: 'left', display: jmxdisplay }}>
					<FormItem label="jmx 用户名" key="username" hasFeedback {...formItemLayout}>
				...
				<span style={{ width: '33%', float: 'left', display: jmxdisplay }}>
					<FormItem label="jmx 密码" key="password" hasFeedback {...formItemLayout}>
