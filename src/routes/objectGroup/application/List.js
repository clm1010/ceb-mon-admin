...
function list ({
 dispatch, loading, dataSource, pagination, batchDelete, selectedRows, controllerNumber, q,
	...
	const onPreview = (record) => {
		window.open(`/rulespreview?ids=${record.mo.uuid}&branches=${record.mo.branchName}&firstClass=APP`, `${record.mo.uuid}`, '', 'false')
	}
	columns.push({
	  	title: '操作',
	  	width: 90,
  ...
  <Button style={{ float: 'left' }} size="small" type="ghost" shape="circle" icon="desktop" onClick={() => onPreview(record)} />
	...
	const onEdit = (record) => {
		dispatch({
			type:'application/appcategories',
			payload:{
				q:'affectSystem=="网络|*"'
			}
		})
		dispatch({
			type: 'application/findById',				//@@@
			payload: {
