...
const application = ({
 location, dispatch, application, appSelect, loading, app, policyList
}) => {
	const {
		q,
		list,
		...
		appCategorlist,
		secondClass,
	} = application					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
	...
	const modalProps = {	//这里定义的是弹出窗口要绑定的数据源
		...
		modalName: secondClass.title,//'应用系统',		//@@@
		_mngInfoSrc,
		alertType,
		alertMessage,
		appSelect,
  ...
  const importResultModalProps = {
  	...
  	queryPath: 'application/setState',
  }
...
export default connect(({
 application, appSelect, app, loading, policyList
}) => ({
 application, appSelect, app, loading, policyList
}))(application) //@@@
