...
//@@@
const line = ({
	app, location, dispatch, line, policyList, interfaceList, interfacer, moSingleSelect, loading, appSelect,
}) => {
	const {
		q,
		list,
		...
		FScloud,
		optionSelectAppName,
	} = line					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
	...
	const filterProps = { //这里定义的是查询页面要绑定的数据源
		...
		queryPreProcess(data) {
			if (data.mo.aaIntf.ip !== undefined) {
				data['aaIntf.ip'] = data.mo.aaIntf.ip
				delete data.mo.aaIntf.ip
			}
			return data
		},
	}
	...
	const listProps = { //这里定义的是表格对应的数据源与配置
		dispatch,
		dataSource: list,
		loading: loading.effects['line/query'],
		pagination,
		key: pageChange,
		q,
		batchDelete,
...
export default connect(({
	app, line, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}) => ({
	app, line, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}))(line) //@@@
