...
const modal = ({
	...
	const onOk = () => {
		...
		validateFieldsAndScroll((errors) => {
			...
			if (modalType === 'create') {
				...
				dispatch({
					type: 'osFs/setState',				//@@@//抛一个事件给监听这个type的监听器
					payload: {
						modalVisible: false,
					},
				})
	...
	//end
	return (
		<Modal {...modalOpts} width={850} key="osFsModal">
			<Form layout="horizontal">
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="用途" key="osFsUse" hasFeedback {...formItemLayout}>{/**待定 */}
						{getFieldDecorator('osFsUse', {
							initialValue: item.keyword,
							rules: [
								{
									// required: true,
