...
//@@@
const dbService = ({
	location, dispatch, dbService, moSingleSelect, loading, appSelect
   }) => {
	const {
		q,
		list,
		...
		thirdClass,
		appCategorlist,
	} = dbService					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['dbService/query'],
    pagination,
    secondClass,
    q,
    batchDelete,
...
export default connect(({ dbService, moSingleSelect, loading, appSelect }) => ({ dbService, moSingleSelect, loading, appSelect }))(dbService)	//@@@
