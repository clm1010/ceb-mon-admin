...
const modal = ({
	...
	if (appSelect.currentItem.c1 && modalType === 'create') {
		...
		// message.warning('应用分类名称不存在！')
			...
			if (modalType === 'create') {
				dispatch({
					type: `dbService/${modalType}`,				//@@@
					payload: {
						currentItem: data,
					},
				})
				dispatch({
					type: 'dbService/setState',				//@@@//抛一个事件给监听这个type的监听器
					payload: {
						modalVisible: false,
					},
				})
			...
			<Form layout="horizontal">
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="类型" key="secondClass" hasFeedback {...formItemLayout}>
						...
						})(<Select disabled="true" defaultValue ={secondClass}>
				...
				<span style={{width:'50%', float:'left' }}>
					<FormItem label="数据库子类型" key="thirdClass" hasFeedback {...formItemLayout}>
						...
						})(<Select  disabled="true" defaultValue = {thirdClass}>
