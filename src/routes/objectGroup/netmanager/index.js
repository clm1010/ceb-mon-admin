...
//@@@
const netmanager = ({
 app, location, dispatch, netmanager, policyList, interfaceList, interfacer, moSingleSelect, loading, appSelect,
}) => {
	const {
		q,
		list,
		...
		snmpVerflag,
		optionSelectAppName,
	} = netmanager					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['netmanager/query'],
    pagination,
    location,
    key: pageChange,
    q,
...
export default connect(({
 app, netmanager, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}) => ({
 app, netmanager, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}))(netmanager)	//@@@
