...
//@@@
const server = ({
 location, dispatch, server, appSelect, loading, app, policyList,
}) => {
	const {
		q,
		list,
		...
		FScloud,
		optionSelectAppName,
	} = server					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['server/query'],
    pagination,
    key: pageChange,
    q,
    batchDelete,
...
export default connect(({
 server, appSelect, app, policyList, loading,
}) => ({
 server, appSelect, app, policyList, loading,
}))(server) //@@@
