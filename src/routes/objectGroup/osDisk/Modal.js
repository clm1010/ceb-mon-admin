...
const modal = ({
	...
	const moSingleSelectProps =
		Object.assign(
			...
			moSingleSelect,
			{
				...
				compName: 'parentMoUuid',
				formItemLayout,
				externalFilter: location === undefined ? '' : ` firstClass=='${firstClass}' and secondClass=='${secondClass}'`,
				defaultValue: modalType === 'update' ? ('belongsTo' in item ? item.belongsTo.name : '查不到所属操作系统') : '',
				onSelect,
				query: queryMo,
			}
		)
	...
	const onOk = () => {
		...
		validateFieldsAndScroll((errors) => {
			...
			if (modalType === 'create') {
				dispatch({
					type: `osDisk/${modalType}`,				//@@@
					payload: {
						currentItem: data,
					},
				})
				dispatch({
					type: 'osDisk/setState',				//@@@//抛一个事件给监听这个type的监听器
					payload: {
						modalVisible: false,
					},
				})
	...
	//end
	return (
		<Modal {...modalOpts} width={850} key="osDiskModal">
			<Form layout="horizontal">
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="类型" key="secondClass" hasFeedback {...formItemLayout}>
						{getFieldDecorator('secondClass', {
							...
							<Select.Option value="OS_WINDOWS">WINDOWS</Select.Option>
							<Select.Option value="OS_LINUX">LINUX</Select.Option>
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="操作系统子类型" key="thirdClass" hasFeedback {...formItemLayout}>
						{getFieldDecorator('thirdClass', {
							...
							<Select.Option value="OS_FS">文件系统</Select.Option>
							<Select.Option value="OS_DISK">磁盘</Select.Option>
						</Select>)
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="用途" key="osDiskUse" hasFeedback {...formItemLayout}>{/**待定 */}
						{getFieldDecorator('osDiskUse', {
							initialValue: item.keyword,
							rules: [
								{
									// required: true,
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="操作系统版本信息" key="version" hasFeedback {...formItemLayout}>
				...
				{/* <span style={{ width: '50%', float: 'left' }}>
					<FormItem label="操作系统实例" key="instance" hasFeedback {...formItemLayout}>
