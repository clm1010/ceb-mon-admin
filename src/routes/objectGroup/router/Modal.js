...
import { Form, Input, Modal, Button, Select, DatePicker, Icon, Alert, Tooltip, message, Divider } from 'antd'
...
const modal = ({
	...
	const onCancel = () => { //弹出窗口中点击取消按钮触发的函数
		resetFields()
		dispatch({
			type: 'router/setState',				//@@@//抛一个事件给监听这个type的监听器
			payload: {
				modalVisible: false,
				FScloud: false,
				snmpVerflag:true,
			...
			<Form layout="horizontal">
				...
				<span style={{ width: '33%', float: 'left' }}>
					<FormItem label="所属行名称" key="branchName" hasFeedback {...formItemLayout}>
						...
						})(<Select onSelect={onBranchNameChange} disabled={user.branch !== undefined || user.branch === 'EB'} filterOption={onSearchInfo} showSearch>{genDictOptsByName('branch')}</Select>)}
					</FormItem>
				</span>
