...
//@@@
const router = ({
 app, location, dispatch, router, policyList, interfaceList, interfacer, moSingleSelect, loading, appSelect,
}) => {
	const {
		q,
		list,
		...
		snmpVerflag,
		optionSelectAppName,
	} = router					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['router/query'],
    pagination,
    location,
    key: pageChange,
    q,
...
export default connect(({
 app, router, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}) => ({
 app, router, policyList, interfacer, interfaceList, moSingleSelect, appSelect, loading,
}))(router) //@@@
