...
//@@@
const dbDataBase = ({
	location, dispatch, dbDataBase, moSingleSelect, loading,appSelect
   }) => {
	const {
		q,
		list,
		...
		thirdClass,
		appCategorlist,
	} = dbDataBase					//@@@//这里把入参做了拆分，后面代码可以直接调用拆分的变量
  ...
  const listProps = { //这里定义的是表格对应的数据源与配置
    dispatch,
    dataSource: list,
    loading: loading.effects['dbDataBase/query'],
    pagination,
    secondClass,
    q,
    batchDelete,
...
export default connect(({ dbDataBase, moSingleSelect, loading, appSelect }) => ({ dbDataBase, moSingleSelect, loading, appSelect }))(dbDataBase)	//@@@
