...
const modal = ({
	...
	const onSelect = (value, option) => {
		...
		item.thirdClass = neParentobj.thirdClass
	...
	const onOk = () => {
		...
		validateFieldsAndScroll((errors) => {
			...
			if (modalType === 'create') {
				dispatch({
					type: `dbDataBase/${modalType}`,				//@@@
					payload: {
						currentItem: data,
					},
				})
				dispatch({
					type: 'dbDataBase/setState',				//@@@//抛一个事件给监听这个type的监听器
					payload: {
						modalVisible: false,
					},
				})
	...
	//end
	return (
		<Modal {...modalOpts} width={850} key="dbDataBaseModal">
			<Form layout="horizontal">
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="用途" key="dbDataBaseUse" hasFeedback {...formItemLayout}>{/**待定 */}
						{getFieldDecorator('dbDataBaseUse', {
