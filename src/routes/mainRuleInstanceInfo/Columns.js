			...
			if (record.filter.filterMode === 'BASIC') { //基础模式
				...
				if (record.filter.basicFilter.distributedFilterItems.length > 0) { //其它域对象选择器
					for (let distributedFilter of record.filter.basicFilter.distributedFilterItems) {
						const appName = distributedFilter.application.name
						const clusters = distributedFilter.application.clusters
						const namespaces = distributedFilter.application.namespaces
						const indicators = distributedFilter.application.indicators
						infoState = `屏蔽的应用是：${appName}`
						if (clusters!==null && clusters!==undefined && clusters.length > 0) {
							const c = clusters.map(item => item.name).join(',')
							infoState += `,屏蔽的集群是：${c}`
						}
						if (namespaces!==null && namespaces!==undefined && namespaces.length > 0) {
							const n = namespaces.map(item => item.name).join(',')
							infoState += `,屏蔽的命名空间是：${n}`
						}
						if (indicators!==null && indicators!==undefined && indicators.length > 0) {
							const i = indicators.map(item => item.name).join(',')
							infoState += `,屏蔽的命名空间是：${i}`
						}
					}				
				}
				...
				let summary = '告警描述为:'//告警详情
				let alterGroup = '告警组为:'//告警组
				let componentType = '告警大类为:' //告警大类
				let servert = '告警级别:' //告警级别
				let Agent = '告警代理:' //告警代理
				if (record.filter.advFilterItems.length > 0) {
					for (let advFilter of record.filter.advFilterItems) {
						...
						} else if (advFilter.field === 'N_ComponentType') { //告警大类
							componentType = `${componentType + advFilter.value},`
						}else if (advFilter.field === 'N_CustomerSeverity') { //告警级别
							servert = `${servert + advFilter.value},`
						}else if (advFilter.field === 'Agent') { //告警级别
							Agent = `${Agent + advFilter.value},`
						}
					}
				}
				advFilterState = (appName === '应用系统为:' ? '' : appName) +
				(nodeAlias === '屏蔽IP为:' ? '' : nodeAlias) +
				(summary === '告警描述为:' ? '' : summary) +
				(alterGroup === '告警组为:' ? '' : alterGroup) +
				(componentType === '告警大类为:' ? '' : componentType) + 
				(servert === '告警级别:' ? '' : servert) +
				(Agent === '告警代理:' ? '' : Agent)
				...
				if (record.filter.advFilterItems.length > 0) {
					for (let advanced of record.filter.advFilterItems) {
						advancedState = `${advancedState} ${advanced.leftBrackets === undefined ? ' ' : `${advanced.leftBrackets} `} ${advanced.field} ${advanced.op} ${advanced.value} ${advanced.rightBrackets === undefined ? ' ' : `${advanced.rightBrackets} `} ${advanced.logicOp === undefined ? ' ' : `${advanced.logicOp} `}`
					}
				}
