	...
	query = () => {
		...
		for (let [key, value] of Object.entries(data)) {
			if (key === 'rule_name' && value != undefined && value != '') {
				...
				q += `mo.name=='*${value}*';`
			} else if (key === 'IP' && value != undefined && value != '') {
				q += `filter.basicFilter.neFilterItems.keyword=='*${value}*';`
			} else if (key === 'appName' && value != undefined && value != '') {
				q += `filter.basicFilter.appFilterItems.name =='*${value}*';`
			} else if (key === 'content' && value != undefined && value != '') {
				q += `filter.advFilterItems.value=='*${value}*';`
			} else {
				switch (typeof (value)) {
					case 'number':
						q += `${key}==${value};`
  ...
  <Form>
    ...
    <Row style={{ marginTop: '10px' }}>
      <Col span={24} style={{ textAlign: 'right' }}>
        <Button htmlType="submit" onClick={this.query} icon="search" />
