...
class Filter extends React.Component {
    ...
    let q_appName = ''
		...
		for (let [key, value] of Object.entries(data)) {
			...
			} else if (key === 'content----' && value !== undefined && value.length > 0) {
				q += `((filter.advFilterItems.value=='*${value}*') or (filter.basicFilter.neFilterItems.keyword=='*${value}*') or (filter.basicFilter.otherFilterItems.name=='*${value}*') or (filter.basicFilter.appFilterItems.name=='*${value}*'));`
			} else if (key === 'accurate----' && value !== undefined && value.length > 0) {
				q += `((filter.advFilterItems.value=='${value}') or (filter.basicFilter.neFilterItems.keyword=='${value}') or (filter.basicFilter.otherFilterItems.name=='${value}') or (filter.basicFilter.appFilterItems.name=='${value}'));`
			} else if (key === 'appName' && value !== undefined && value.length > 0) {
        q_appName = `((appNameAuto=='*${value.toString().indexOf('（') === -1 ? value : value.toString().substring(0, value.toString().indexOf('（'))}*') or (appNameEditing=='*${value.toString().indexOf('（') === -1 ? value : value.toString().substring(0, value.toString().indexOf('（'))}*') `
			} else if (key === 'accurate') {
			  value === 'true' ? (q_appName === '' || q_appName === undefined) ? '' : (q += `${q_appName});`) : (q_appName === '' || q_appName === undefined) ? '' : (q += `${q_appName}or (appNameEditing=='全局'));`)
      }
			//普通控件，查询条件普通处理
			else {
				switch (typeof (value)) {
		...
		for (let i = 0; i < schema.length; i++) {
			...
			switch (schema[i].showType) {
				...
				case 'normalCondition':
					children.push(this.transformAddCol(schema[i], i, getFieldDecorator))
					break
	...
	transformMultiSelect (field, key, getFieldDecorator) {
		...
		return (<Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 24 }} key={key}><FormItem {...formItemLayout} label={field.title}>{getFieldDecorator(field.key, { initialValue: field.defaultValue })(<Select allowClear mode="multiple" placeholder={field.placeholder || '请选择'}>{options}</Select>)}
                                                                             </FormItem>
          </Col>)
	}
	...
	transformTreeSelect (field, key, getFieldDecorator) {
		...
		return (<Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 24 }} lg={{ span: 8 }} key={key}><FormItem {...formItemLayout} label={field.title}>{getFieldDecorator(field.key, { initialValue: field.defaultValue })(<TreeSelect onChange={moChange} getPopupContainer={() => document.body} style={{ width: '100%' }} size="default" dropdownStyle={{ maxHeight: '300px', overFlow: 'scroll' }} placeholder={field.placeholder}>{treeNodes}
                                                                                                                                                                                                                    </TreeSelect>)}
                                                                                              </FormItem>
          </Col>)
	}
	...
	transformAddCol(field, key, getFieldDecorator) {
		return (
			<>
				<Col xl={{ span: 24 }} md={{ span: 24 }} sm={{ span: 24 }} key={'dashed'}>
					<hr style={{ height: '1px', borderStyle: 'dashed', opacity: 0.5 }} />
				</Col>
				<Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 8 }} key={key}>
					<FormItem {...formItemLayout} label={field.title}>
						{getFieldDecorator(field.key, { initialValue: field.defaultValue })(<Input placeholder={field.placeholder || '请输入'} />)}
					</FormItem>
				</Col>
			</>
		)
  ...
  searches = (value) => {
    if (value != '') {
      let q = `affectSystem=='*${value}*'`
      this.state.dispatch({
        type: 'mainRuleInstanceInfo/queryOptionSelectAppName',
        payload: {
          q,
		  page:0,
		  pageSize:200
  ...
  transformAsySelect (field, key, getFieldDecorator) {
    return (<Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 24 }} key={key}>
      <FormItem {...formItemLayout} label={field.title}>
        {getFieldDecorator(field.key, { initialValue: '' })(<Select
          showSearch
          allowClear
          placeholder="请输入应用系统"
