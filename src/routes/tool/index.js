...
const tools = ({
	location, dispatch, tool, loading, labelGroup,
}) => {
	const {
	...
	} = tool	//这里把入参tool做了拆分，后面代码可以直接调用拆分的变量
	...
	const mosListProps = {
		...
		scroll: 3500,
		columns: MosColumns,
		dataSource: tool.mosList,
		pagination: tool.paginationMos,
		onPageChange(page) {
			dispatch({
				type: 'tool/queryMos',
				payload: {
	...
	const mosModalDescProps = {
		dispatch,
		visible: tool.mosVisible,
		policyListProps: mosListProps,
		loading,
		titlename: '关联监控对象',
		policyObjInfo: `关联监控对象数 ${tool.toolMosNumber}`,
	...
	const lableinfo = {
		...
		lablepagination,
		lableInfoVal,
		loading: loading.effects['tool/lablequery'],
...
export default connect(({ tool, labelGroup, loading }) => ({ tool, labelGroup, loading: loading }))(tools)
