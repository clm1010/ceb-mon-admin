	...
	function findElem(elem, arr) {
		...
		arr.forEach((item) => {
			if (item.key == elem.key) {
				flag = false
				item.name = elem.name
				item.alias = elem.name
			}
		})
		return flag
	}

	ViewColumns.forEach((item) => { //更新数据字典里面的值为最新的结果
		if (findElem(item, columeList)) {
			let colume1 = {
				key: item.key,
				name: item.name,
				alias: item.name,
							...
							<div className="col-part-left">
								...
								<Col span={11}>
									...
									<div className="col-button-02">
										<Button onClick={upTop}><Icon type="vertical-align-top" /></Button>
										<Button onClick={up}><Icon type="up" /></Button>
										<Button onClick={down}><Icon type="down" /></Button>
										<Button onClick={downTop}><Icon type="vertical-align-bottom" /></Button>
									</div>
								</Col>
							</div>
