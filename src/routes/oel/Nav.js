...
import { Row, Col, Button, Select, Tooltip, Tag, Divider, Icon, Badge } from 'antd'
import './navStyle.css'
const { Option, OptGroup } = Select

function nav({
	dispatch, loading, filterDisable, dataSource, pagination, showEventFilterList, location, tagFilters, currentSelected, filteredSeverityMap, showdataSouseList, showviewList, oelViewer, oelColumns, oelDatasource, oelFilter, orderBy, preSearch,
		...
		<Row gutter={24} style={{ backgroundColor: 'white' }}>
			<Col xl={{ span: 24 }} md={{ span: 24 }} sm={{ span: 24 }}>
				<div style={{ backgroundColor: 'white', marginTop: 8, marginBottom: 8 }}>
					...
					<span className="glqjg">
						<Tooltip title="告警压缩视图">
							<Button icon="vertical-align-middle" onClick={() => {
								window.open(`${window.location.origin}/oelCompression${preSearch}`, '告警压缩')
							}} >告警压缩视图</Button>
						</Tooltip>
					</span>
