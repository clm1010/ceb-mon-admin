 ...
 * @module 监控配置/监控指标管理 
 * @description 
 * URL: <u>/stdIndicatorGroup/stdIndicatorInfo</u>
 *
 * 此页面用于添加、修改和删除指标组及指标。
...
import StdInfo from '../stdIndicatorsInfo'
...
function StdIndicatorgroup ({
 children, location, dispatch, stdIndicatorGroup, stdIndicatorsinfo, loading,
}) {
	const {
 treeDatas, selectTreeNode, selectKeys, modalVisible, modalType, isClose,
} = stdIndicatorGroup
	const { expand } = stdIndicatorsinfo
  const stdinfoProps = {
    location,
    dispatch,
    stdIndicatorGroup,
	...
	const onSelect = (selectedKeys, info) => {
		...
		dispatch({ //把选择中的 key 保存到state中，以便在其他触发事件中使用
			type: 'stdIndicatorGroup/controllerState',
			payload: {
		...
		dispatch(routerRedux.push(`/stdIndicatorGroup/stdIndicatorInfo`)) //显示第三级页面
	}
  ...
  <Row gutter={12} className="content-tree">
    ...
    <Col lg={expand ? 19 : 24} md={expand ? 18 : 24} sm={expand ? 17 : 24} xs={24} className="content-right">
      <div className="lines" />
      <Switch>
        <Route path='/stdIndicatorGroup/stdIndicatorInfo' render={() => (<StdInfo {...stdinfoProps}/>)}/>
      </Switch>
    </Col>
  </Row>
	)
...
StdIndicatorgroup.propTypes = {
  StdIndicatorgroup: PropTypes.object,
}

//export default connect()(StdIndicators)
export default connect(({ stdIndicatorGroup, stdIndicatorsinfo, loading }) => ({ stdIndicatorGroup, stdIndicatorsinfo, loading: loading.models.stdIndicatorGroup }))(StdIndicatorgroup)
