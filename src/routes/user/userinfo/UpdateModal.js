...
import { Form, Input, Row, Modal, Table, TreeSelect, Select, Cascader, Radio } from 'antd'
...
const modal = ({
  ...
  const onOk = () => {	//点击确定提交数据
    validateFields((errors) => {
      ...
      if (type === 'update') { //点击修改按钮
        let roles = []
        roleUUIDs.forEach((item) => { //角色信息
          let value = {
            uuid: item.value,
          }
  ...
  const onCancel = () => {	//点击取消清除数据，并还原数据形式
    resetFields()
    dispatch({
  ...
  const modalOpts = {
    title: `${type === 'create' ? '用户信息新增' : '用户信息编辑'}`,
    visible,
    onOk,
    onCancel,
        ...
        <Row>
          <div style={{ float: 'left' }}>
            ...
            <span style={{ float: 'right', marginTop: 2, marginLeft: 10 }}>
              <div style={{ position: 'relative' }} id="areae" />
              <FormItem label="机　　构" {...formItemLayout}>
                {getFieldDecorator('branch', {
                  ...
                  <Select
                    ...
                    getPopupContainer={() => document.getElementById('areae')}
                    style={{ width: '180px' }}
                  >
          ...
          <div style={{ float: 'left' }}>
            <div style={{ position: 'relative' }} id="areaq" />
            <span style={{ float: 'left', marginTop: 2 }}>
              <FormItem label="用户状态" {...formItemLayout}>
                {getFieldDecorator('status', {
                  ...
                  style={{ width: '180px' }}
                  showSearch
                  //getPopupContainer={() => document.getElementById('areaq')}
                >
            ...
            <span style={{ float: 'right', marginTop: 2 }}>
              <FormItem label="修改时间" {...formItemLayout}>
                {getFieldDecorator('updatedTime', {
                  initialValue: item.updatedTimeStr,
                  rules: [{ required: true }],
                })(<Input disabled style={{ width: '180px' }} />)}
              </FormItem>
            </span>
          ...
          <div style={{ float: 'left', marginLeft: 6 }}>
            <div style={{ position: 'relative', overFlow: 'scroll' }} id="area12" />
            <FormItem label="角色" {...formItemLayout3}>
              {
                getFieldDecorator('selectRole', {
                  ...
                  {...tProps}
                  getPopupContainer={() => document.getElementById('down1')}
                />)
              }
            </FormItem>
          </div>
