...
for (let column of ViewColumns) {
	SelectFilters.push(<Select.Option key={column.name} value={column.key}>{column.name}</Select.Option>)
}
...
const ConditionBasicMode = ({
 ...
 leftBrackets: '', field: '', op: '', value: '', rightBrackets: '', logicOp: '',
	...
	let {
 filterMode = 'BASIC',
		basicLogicOp = filter.basicLogicOp,
  ...
  const onFocus = (index) => {
	...
	const mySearchInfo = (input, option) => {
		...
		//let index = 0
  const loop = data => data.map((item0, index) => {
	  //index = index + 1
	switch (filterMode) {
	  case 'BASIC':
		return (
        ...
        <FormItem {...FormItemProps} hasFeedback key={`field_info_${filterIndex[index]}`}>
          {
						getFieldDecorator(`field_${filterIndex[index]}`, {
							initialValue: item0.field,

						})(<Select onChange={saveCurrent} showSearch filterOption={mySearchInfo} getPopupContainer={() => getPopupContainers(index)}>
  {SelectFilters}
         </Select>)
					}
        </FormItem>
      ...
      <FormItem {...FormItemProps} hasFeedback key={`op_info_${filterIndex[index]}`}>
        {
						getFieldDecorator(`op_${filterIndex[index]}`, {
							initialValue: item0.op,
						})(<Select onChange={saveCurrent} style={{ width: '100%' }}>
  {SelectOP}
         </Select>)
					}
      </FormItem>
    </Col>
    <Col key={`col_${filterIndex[index]}_3`} {...ColProps} xl={{ span: 8 }} md={{ span: 7 }}>
      <FormItem {...FormItemProps} hasFeedback key={`value_info_${filterIndex[index]}`}>
        {
						getFieldDecorator(`value_${filterIndex[index]}`, {
							initialValue: item0.value,

						})(<Input onChange={saveCurrent} />)
					}
      </FormItem>
    </Col>

    <Col key={`col_${filterIndex[index]}_6`} style={{ textAlign: 'right' }} {...ColProps} xl={3} md={4}>
      ...
      <FormItem {...FormItemProps} hasFeedback key={`leftBrackets_info_${filterIndex[index]}`}>
        {
						getFieldDecorator(`leftBrackets_${filterIndex[index]}`, {
							initialValue: item0.leftBrackets,
      ...
      <FormItem {...FormItemProps} hasFeedback key={`rightBrackets_info_${filterIndex[index]}`}>
        {
						getFieldDecorator(`rightBrackets_${filterIndex[index]}`, {
							initialValue: item0.rightBrackets,
