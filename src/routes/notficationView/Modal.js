...
import { Form, Input, Modal, Switch, Button, Select, Transfer, Alert, Spin, message } from 'antd'
...
const modal = ({
	...
	//拼装应用options
	let optionsInfo = []
	if (AppOption !== undefined) {
		AppOption.forEach((item, index) => { //添加name属性--> 将名字改掉   其实是传的uuid
			let values = item.affectSystem
			let uuid = item.uuid
			let englishCode = item.c1
			optionsInfo.push(<Option key={englishCode} uuid={englishCode} value={values}>{values}</Option>)
		})
	}
	...
	const moSelectProps = Object.assign({}, moSelect, {
		placeholders: '请输入设备IP或名字查询，支持多选', name: '设备', modeType: 'multiple', required: false, dispatch, form, item, disabled: notificationType !== 'ORDINARY', compName: 'moUuid', cDefaultName: notificationType === 'ORDINARY' ? info : '每个设备',
	})
	//应用的控件
	...
	const onOk = () => {
		validateFieldsAndScroll((errors, values) => {
			...
			if ((AppUuid.length === 0 && data.moUuid.length === 0) || (data.apps === undefined && data.moUuid.length === 0)) {
				message.info('通知规则中,应用与监控对象至少选择一种！')
			} else {
				dispatch({
	...
	const modalOpts = {
		title: (modalType === 'create' && see === 'no' ? `新增${modalName}` : modalType === 'update' && see === 'no' ? `编辑${modalName}` : modalType === 'update' && see === 'yes' ? `查看${modalName}` : null),
				...
				<span style={{ width: '50%', float: 'left' }}>
					<FormItem label="类型" key="informType" {...formItemLayout1}>
						...
						>
							...
							{/*<Option value="ORDINARY">普通通知</Option>*/}
							{/*<Option value="ALL">全局通知</Option>*/}
						</Select>)}
					</FormItem>
				</span>
