...
import { Row, Col, Card, Table, Tree, Badge, Progress, Tooltip, Select, Button, message, Tag, Popover, Icon, DatePicker } from 'antd'
...
const performance = ({
  dispatch, loading, location, performance, initValue, countState, monitor,
}) => {
  ...
  const faultType = [
    {
      ...
      render: (text, record) => {
        ...
        return <Link to={id} target="_blank" rel="opener">{text}</Link>
      },
    },
  ]
  ...
  return (
    <Row gutter={24}>
      ...
      <Col lg={24} md={24} sm={24} xs={24}>
        <Row style={{ height: "auto",position: "relative"}}>
          <Col lg={6} md={6} sm={6} xs={6} style={left}>
          ...
          <Col lg={18} md={18} sm={18} xs={18} style={right}>
            <Card bodyStyle={{ width: '98%', border: 'none', padding: 6 }}
              style={{ marginBottom: '6px' }}
              title="最新100条告警"
              extra={
        ...
        </Card>
        {
          user.branch === 'ZH' || user.branch === '' || user.branch === undefined ?
            <Card bodyStyle={{
              ...
              style={{ marginBottom: '6px' }}
              title="output discard Top 20 / input discard Top 20"
            >
              <div style={{ width: '49%', float: 'left' }}>
                <Table
        ...
        }
        {
          user.branch === 'ZH' || user.branch === '' || user.branch === undefined ?
            <Card bodyStyle={{
              ...
              style={{ marginBottom: '6px' }}
              title="output CRC error Top 20 / input CRC error Top 20"
            >
              <div style={{ width: '49%', float: 'left' }}>
                <Table
...
export default connect(({ performance, interfaces, monitor, loading }) => ({ performance, interfaces, monitor, loading }))(performance)
