...
import { Form, Button, Select, Row, Col, Alert, Input } from 'antd'
...
const filterForm = ({
	dispatch, form, org, deviceType, vendor, netDomain, discoveryIP, appName, name
}) => {
	const {
	...
	const onOk = () => {
		validateFieldsAndScroll((errors, value) => {
			...
			if (data.netDomain) {
				if (data.netDomain.length > 0 && data.appName.length == 0) {
					let qs = '('
					appNameOpt.forEach((item) => {
						qs += `appName==${item.key} or `
					})
	...
	let infos = (org === '' ? '' : `检索设备的分行属性为：${org}, `) + (deviceType === '' ? '' : `设备类型为：${deviceType}, `) + (vendor === '' ? '' : `设备厂商为：${vendor}, `) +
		(discoveryIP === '' ? '' : `设备IP为：${discoveryIP}`) + (netDomain === '' ? '' : `设备网络域为：${netDomain}, `) + (appName.length === 0 ? '' : `应用系统名字为：${appName}`)
	if (infos === '') {
		infos = '请输入搜索条件'
	}
	let options = []
	...
	let netDomainOpt = genDictOptsByName('netdomin')
	let appNameOpt = []
	if (netDomain && netDomain !== '') {
		let net_app = genDictOptsByName('netdomin-appname')
		net_app.filter((currentValue) => {
			if (currentValue.props.children == netDomain) return true
		})[0].props.value.split(',')
			.forEach(opt => {
				appNameOpt.push(<Option key={opt} value={opt}>{opt}</Option>)
			});
	}
	return (
					...
					<Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 8 }} xm={{ span: 8 }}>
						<FormItem label="网络域" key="netDomain" hasFeedback {...formItemLayout}>
							{getFieldDecorator('netDomain', {
								initialValue: [],
							})(<Select allowClear showSearch filterOption={onSearchInfo} placeholder="请选择" onChange={netDomainOnSelect}>
								{netDomainOpt}
							</Select>)}
						</FormItem>
					</Col>
					<Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 8 }} xm={{ span: 8 }}>
						<FormItem label="域内分区" key="appName" hasFeedback {...formItemLayout}>
							{getFieldDecorator('appName', {
								initialValue: [],
							})(<Select allowClear showSearch filterOption={onSearchInfo} placeholder="请选择" mode='multiple' onChange={appNameOnSelect}>
								{appNameOpt}
							</Select>)}
						</FormItem>
					</Col>
					<Col xl={{ span: 8 }} md={{ span: 8 }} sm={{ span: 8 }} xm={{ span: 8 }}>
						<FormItem label="设备名称" key="name" hasFeedback {...formItemLayout}>
							{getFieldDecorator('name', {
							})(<Input />)}
						</FormItem>
					</Col>
					<Col xl={{ span: 16 }} md={{ span: 16 }} sm={{ span: 16 }} xm={{ span: 16 }}>
