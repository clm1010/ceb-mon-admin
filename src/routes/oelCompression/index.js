...
import SubAlarmList from './SubAlarmList'
...
import EventFilterModal from '../oel/OELEventFilter/EventFilterModal'
import ToolSet from '../oel/toolSet/ToolSet'
import ToolEdit from '../oel/toolSet/ToolEdit'
import ToolEd from '../oel/toolSet/ToolEd'

import ViewSet from '../oel/view/ViewSet'
import CopyView from '../oel/view/CopyView'
import ColumeEdit from '../oel/view/ColumeEdit'

import DataSouseSet from '../oel/dataSouse/DataSouseSet'
import DataSouseEdit from '../oel/dataSouse/DataSouseEdit'
import CopyDataSouse from '../oel/dataSouse/CopyDataSouse'
...
const oelcompression = ({
 location, dispatch, oelcompression, oelEventFilter, loading, oelToolset, oelDataSouseset, eventviews,
}) => {
	const {
    knowledges, title, filterDisable, list, pagination, currentItem, initValue, tagFilters, currentSelected, visibleFilter, visibleDetail, dataSouseeditVisible, oelViewer, oelColumns, oelDatasource, toolList, oelFilter, currentFieldValue, displayObsSrvsList, filteredSeverityMap, filterList, datasourceList, viewList, countState, orderBy, selectedRows, cfgType, woCurrentItem, alertType, alertMessage, processEventVisible, contextType, contextMessage, contextInput, workOrderVisible, fieldKeyword, journelList, curDetailTabKey,
    isPackedAlarms, treeNodes, visibleSubAlarms, subAlarms, subPagination, subSelectedRows, processEventVisibleSub, workOrderVisibleSub, selectedKeys, treeCollapse,visibleSearchFilter ,appNameList, fetchingApp,appNameNum,Service_Impact_Num,HotPotNum,callOutList,callUsers
} = oelcompression
  ...
  const listProps = { //这里定义的是查询页面要绑定的数据源
    dispatch,
    dataSource: list,
    loading: loading.effects['oelcompression/query'],
    pagination,
    location,
    tagFilters,
    currentSelected,
  ...
  const modalSubAlarmProps = {
  	dispatch,
    dataSource: subAlarms,
    ...
    toolList,
    user,
    selectedRows:subSelectedRows,
 	  visibleSubAlarms,
  	subAlarms,
    modalDetailProps:{
      dispatch,
  	  //visibleDetail:subAlarmVisibleDetail,
    ...
    modalProcessEventProps: {
      dispatch,
      selectedRows:subSelectedRows,
      visible: processEventVisibleSub,
      oelDatasource,
      contextType,
      contextMessage,
      callUsers,
  ...
  return (
    <div>
      ...
      <SubAlarmList {...modalSubAlarmProps} />
...
export default connect(({
 oelcompression, oelEventFilter, oelToolset, oelDataSouseset, eventviews, loading,
}) => ({
  oelcompression, oelToolset, oelEventFilter, oelDataSouseset, eventviews, loading,
}))(oelcompression)
