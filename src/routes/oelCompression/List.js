...
import { Table, Row, Col, Icon, message,Badge , Button} from 'antd'
...
const followAlarm = (e, data) => {
  ...
  const givenForceCloseAlarm = (e, data) => {
	...
	const getResultRecommend = (e,data) =>{
		...
		dispatch({
			type:'oelcompression/getRecommendAddres',
			payload:{
				AlarmID
			}
		})
	...
	const initialNotification = (e,data) =>{
		dispatch({
			type:'oelcompression/getOutcallUser',
			payload:{
				noticeType:1,
	...
	const filterClick = (e, data) => {
		...
		if (data.upperName === 'Severity') {
			dispatch({
			  type: 'oelcompression/querySuccess',
			  payload: {
			  	tagFilters,
	...
	const showDetail = (record, index, event) => {
		...
		const unfoldCol = (record) =>{
			dispatch({
					type: 'oelcompression/updateState',													//抛一个事件给监听这个type的监听器
					payload: {
						visibleSubAlarms: true,
						alarmSelected: record,
					},
				})
			dispatch({
					type: 'oelcompression/querySubAlarms',													//抛一个事件给监听这个type的监听器
					payload: {
						alarmSelected: record,
				pagination: { current: 1 },
					},
 ...
 return <ContextMenuTrigger collect={collect} upperName={col.key} name={col.dataIndex} id="simple" OZ_AlarmID={record.OZ_AlarmID} record={record} holdToDisplay={1000}><div title={record[col.dataIndex]} className="oel-ellipsis">{record[col.dataIndex]}</div></ContextMenuTrigger>
	...
	if (isPackedAlarms) {
	  oelColumns.forEach(item => {
		if (!item.fixed && addFlag) {
		  showCols.push({
			key: 'action',
			title: "展开",
			//fixed:"left",
  //          render: (text, record) => {return (record.oz_groupid&&record.oz_groupid.length>0)?(<Button onClick={unfoldCol.bind(this,record)} type="link" shape="circle" size='small'>
			render: (text, record) => {return (record.groupNum)?(<Button onClick={unfoldCol.bind(this,record)} type="link" shape="circle" size='small'>
			  <p style={{color:'black',fontSize:'20px'}}>+{record.groupNum}</p>
			  </Button>):null},
			width:60
		   });
		  addFlag = false
