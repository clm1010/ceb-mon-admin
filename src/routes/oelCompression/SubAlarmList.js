...
const followAlarm = (e, data) => {
  ...
  const givenForceCloseAlarm = (e, data) => {
	...
	const triggerTool = (e, data) => {
		...
		if (data.type === 'SQL') {
			...
			dispatch({
			  type: 'oelcompression/sqlTrigger',
			  payload: {
			  	sql: [sql],
			  	uuid: oelDatasource,
			  },
				...
				if (dateFields.indexOf(col.dataIndex) >= 0) {
					...
					col.render = (text, record) => {
						...
						return <ContextMenuTrigger collect={collect} upperName={col.key} name={col.dataIndex} id="subalarm" OZ_AlarmID={record.OZ_AlarmID} record={record} holdToDisplay={1000}>
							<div title={record[col.dataIndex]} className="oel-ellipsis" style={{ background: backgroundColor, color }}>{oz_knowledge_flag}{n_CustomerSeverity}</div></ContextMenuTrigger>
			    	}
 ...
 return <ContextMenuTrigger collect={collect} upperName={col.key} name={col.dataIndex} id="subalarm" OZ_AlarmID={record.OZ_AlarmID} record={record} holdToDisplay={1000}><div title={record[col.dataIndex]} className="oel-ellipsis">{record[col.dataIndex]}</div></ContextMenuTrigger>
  ...
  const onPageChange = (page, filters, sorter) => {
    ...
    dispatch({
      type: 'oelcompression/querySubAlarms',
      payload: {
        pagination: page,
        oelDatasource,
        oelViewer,
	...
	let scrollX = 0
  const showCols = []//oelColumns.slice()//[...oelColumns]
