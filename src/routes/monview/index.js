...
const monview = ({
 dispatch, monview, loading, location, oel,
}) => {
  const {
 line1Olet, line2Olet, line3Olet, modalVisible, modalName, initValue, countState, line1State, line2State, line3State, sourceState,
} = monview
  ...
  const genHtml = (chartList) => {
  	...
  	for (let chart of chartList) {
  		...
  		if (chart.data.length > 0) {
			...
			if (chart.type === 'bar') {
	  		chartHtml.push(<Col lg={5} md={5} key={chart.title} style={{ background: '#fff', margin: '6px', border: `1px solid ${borderColor}` }} onClick={() => window.open(`/oel?oelFilter=${chart.oelFilter}&oelViewer=${chart.oelViewer}&filterDisable=true&title=${chart.title}`, `${chart.oelFilter}`, '', 'false')}>
  <BarChart {...chartProps} />
</Col>)
  		}
  	}
  ...
  return (
    <div>
      ...
      <div style={customPanelStyle}>
        <Row gutter={6}>
          <Spin tip="正在更新 监控维护告警..." spinning={line1State}>{line1Html}</Spin>
        </Row>
      </div>
    </div>
  )
...
export default connect(({ oel, monview, loading }) => ({ oel, monview, loading: loading.models.monview }))(monview)
