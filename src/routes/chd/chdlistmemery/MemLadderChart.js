			...
			onOk () {
				...
				if (ladderChart.allTime.length > 0) {
					for (let i = 0; i < ladderChart.allTime.length; i++) {
						...
						if(flag == '1'){
							info.min = ladderChart.minInfoMem[i]
							info.max = ladderChart.maxInfoMem[i]
							info.avg = ladderChart.avgInfoMem[i]
						}
						if(flag0 == '1'){
							info.min0 = ladderChart.minInfoMem0[i]
							info.max0 = ladderChart.maxInfoMem0[i]
							info.avg0 = ladderChart.avgInfoMem0[i]
						}
						if(flag1 == '1'){
							info.min1 = ladderChart.minInfoMem1[i]
							info.max1 = ladderChart.maxInfoMem1[i]
							info.avg1 = ladderChart.avgInfoMem1[i]
						}
						if(flag2 == '1'){
							info.min2 = ladderChart.minInfoMem2[i]
							info.max2 = ladderChart.maxInfoMem2[i]
							info.avg2 = ladderChart.avgInfoMem2[i]
						}
						if(flag3 == '1'){
							info.min3 = ladderChart.minInfoMem3[i]
							info.max3 = ladderChart.maxInfoMem3[i]
							info.avg3 = ladderChart.avgInfoMem3[i]
						}
						if(flag4 == '1'){
							info.min4 = ladderChart.minInfoMem4[i]
							info.max4 = ladderChart.maxInfoMem4[i]
							info.avg4 = ladderChart.avgInfoMem4[i]
						}
						if(flag5 == '1'){
							info.min5 = ladderChart.minInfoMem5[i]
							info.max5 = ladderChart.maxInfoMem5[i]
							info.avg5 = ladderChart.avgInfoMem5[i]
						}
						if(flag6 == '1'){
							info.min6 = ladderChart.minInfoMem6[i]
							info.max6 = ladderChart.maxInfoMem6[i]
							info.avg6 = ladderChart.avgInfoMem6[i]
						}
						if(flag7 == '1'){
							info.min7 = ladderChart.minInfoMem7[i]
							info.max7 = ladderChart.maxInfoMem7[i]
