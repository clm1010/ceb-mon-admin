...
function list({
	...
	const getResultRecommend = (e, data) => {
		...
		dispatch({
			type: 'oelhint/getRecommendAddres',
			payload: {
				AlarmID
			}
		})
	...
	const unfoldCol = (record) => {
		let clone = record.children ? JSON.parse(JSON.stringify(record.children)) : []
		let alarms = JSON.parse(JSON.stringify(clone))
		...
		oelColumns.forEach(item => {
			if (!item.fixed && addFlag) {
				showCols.push({
					...
					render: (text, record) => {
						if (record.children) {
							let total = record.children.length + 1
							...
							return <Button style={{backgroundColor:'#52c41a',color:'#ffffff'}} size='small' onClick={unfoldCol.bind(this, record)}>{total}</Button>
						}
						return null
					},
					width: 170
