...
{/*     <Tooltip title="压缩告警">
      <Badge overflowCount={99999} count={this.props.packedNumber?this.props.packedNumber:0}  style={{ backgroundColor: '#fff', color: '#999', boxShadow: '0 0 0 1px #d9d9d9 inset' }} showZero />&nbsp;
    </Tooltip> */}
    <Tooltip title="所有告警">
      <Badge overflowCount={99999} count={this.filteredSeverityMap.get('all')?this.filteredSeverityMap.get('all'):0} style={{ backgroundColor: 'black' }} showZero />
    </Tooltip>&nbsp;：&nbsp;
    <Tooltip title="N_CustomerSeverity : 1">
      <Badge overflowCount={99999} count={this.filteredSeverityMap.get('s1')?this.filteredSeverityMap.get('s1'):0} style={{ backgroundColor: '#ed433c' }} showZero />&nbsp;
    </Tooltip>
    <Tooltip title="N_CustomerSeverity : 2">
      <Badge overflowCount={99999} count={this.filteredSeverityMap.get('s2')?this.filteredSeverityMap.get('s2'):0} style={{ backgroundColor: '#f56a00' }} showZero />&nbsp;
    </Tooltip>
    <Tooltip title="N_CustomerSeverity : 3">
      <Badge overflowCount={99999} count={this.filteredSeverityMap.get('s3')?this.filteredSeverityMap.get('s3'):0} style={{ backgroundColor: '#febe2d' }} showZero />&nbsp;
    </Tooltip>
    <Tooltip title="N_CustomerSeverity : 4">
      <Badge overflowCount={99999} count={this.filteredSeverityMap.get('s4')?this.filteredSeverityMap.get('s4'):0} style={{ backgroundColor: '#1f90e6' }} showZero />&nbsp;
    </Tooltip>
    <Tooltip title="N_CustomerSeverity : 100">
      <Badge overflowCount={99999} count={this.filteredSeverityMap.get('s100')?this.filteredSeverityMap.get('s100'):0} style={{ backgroundColor: 'purple' }} showZero />&nbsp;
    </Tooltip>：
    <Tooltip title="已恢复告警">
      <Badge overflowCount={99999} count={this.filteredSeverityMap.get('s0')?this.filteredSeverityMap.get('s0'):0} style={{ backgroundColor: '#52c41a' }} showZero />&nbsp;
    </Tooltip>&nbsp;&nbsp;&nbsp;
  </span>
</div>)
	}
