...
function treelist({
  ...
  isPackedAlarms, treeNodes, selectedKeys, treeCollapse, appNameNum, modalDetailProps, treeTip
}) {
  const onSelect = (v, e) => {
    dispatch({
      type: 'oelhint/query',
  ...
  const loop = data => data.map((item) => {
    let title1
    if(item.lable === '全部'){
      title1= `${item.lable}${item.children.length}个场景`
    }else{
  ...
  treenodes = loop(treeTip)

  const listProps = { //这里定义的是查询页面要绑定的数据源
    dispatch,
    dataSource,
    loading,//.effects['oelhint/query'],
    pagination,
    location,
    tagFilters,
    currentSelected,
