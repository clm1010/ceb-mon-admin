	...
	const onOk = () => { //弹出窗口点击确定按钮触发的函数
		validateFieldsAndScroll((errors) => {
			...
			tabstate.panes.forEach((infos) => {
				infos.content.filterItems.forEach((item)=>{
				...
				let conditions =[]
				if(infos.content.exprForFrontend){
					let aa = eval("("+infos.content.exprForFrontend+")")
					aa.forEach((item)=>{
						let kpi={}
						kpi.indicator={}
				...
				let op = {
					...
					conditions:infos.content.mode=='0' ? infos.content.filterItems : conditions,
					logicOp:infos.content.mode=='0' ? infos.content.logicOp : '',
					mode:infos.content.mode,
					expr:infos.content.expr,
					exprForFrontend:infos.content.exprForFrontend,
