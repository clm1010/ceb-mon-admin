  ...
  const columns = [
    {
		...
		render: (text, record) => {
      	...
      	title: '是否标准',
      	dataIndex: 'instance.policy.stdAlarmFlag',
      	key: 'instance.policy.stdAlarmFlag',
      	render: (text, record) => {
      	  let typename = '未知'
			    if (record.instance.policy.stdAlarmFlag === 1) {
				    typename = '是'
			    } else if (record.instance.policy.stdAlarmFlag === 0) {
				    typename = '否'
			    }
			    return typename
		   },
  ...
  <Button size="default" type="ghost" shape="circle" icon="delete" onClick={() => onDeletes(record)} />
  	...
  	const columns2 = [
    {
		 ...
		 */
      	...
      	title: '是否标准',
      	dataIndex: 'policy.stdAlarmFlag',
      	key: 'policy.stdAlarmFlag',
      	render: (text, record) => {
      	  let typename = '未知'
			    if (record.policy.stdAlarmFlag === 1) {
				    typename = '是'
			    } else if (record.policy.stdAlarmFlag === 0) {
				    typename = '否'
			    }
			    return typename
		   },
