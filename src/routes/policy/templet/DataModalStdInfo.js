...
const modal = ({
	...
	const onOk = () => {
		validateFields((errors) => {
			...
			*/
			for (let filter of filter.filterItems) {
				if (filter.index == selectIndex) {
					filter.indicator.uuid = selectItemObj.uuid
					filter.indicator.name = selectItemObj.name
			  }
			}
			getKpiuuid = selectItemObj
			getKpiuuid.uuid = `#{${selectItemObj.uuid}}_${new Date().getTime()}`
			if(modaType=='basices'){
				dispatch({
			...
			}else if(modaType=='advanced'){
				dispatch({
					type:`${fileType}/controllerModalPlus`,
					payload:{
						exprForFrontend:getKpiuuid,
						kpiVisible: false,
					}
				})
			}
