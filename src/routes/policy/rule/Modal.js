...
const modal = ({
	...
	const onOk = () => {																				//弹出窗口点击确定按钮触发的函数
		validateFields((errors) => {
			...
			let payload = {
				...
				tagUUIDs: label,
				std: data.std,
				isBasics: typeValue == 'DISTRIBUTE' ? data.isBasics : 0
			}
			//监控对象特征验证---start
			let valiMo = true
			...
			// payload.filters[0].filterItems[payload.filters[0].filterItems.length - 1].logicOp = ''
			if(payload.filters[0].filterItems){
				payload.filters[0].filterItems[payload.filters[0].filterItems.length - 1].logicOp = ''
			}
			if (payload.filters[0].thirdClass && payload.filters[0].thirdClass === '' || payload.filters[0].thirdClass == undefined) {
				delete payload.filters[0].thirdClass
			}
			//end
			if (valiMo) {
				Modal.warning({
	...
	*/
	...
	const TreeSelectValue = loopSelect1(treeData)
	return (
		<Modal {...modalOpts}
			width="600px"
					...
					}
					{
						typeValue == 'DISTRIBUTE' ?
							<div style={{ position: 'relative' }} id="area4" >
								<FormItem label="规则类型" hasFeedback {...formItemLayout}>
									{getFieldDecorator('isBasics', {
										initialValue: item.isBasics,
										rules: [{ required: true }],
									})(<Radio.Group>
										<Radio value={1}>基础规则</Radio>
										<Radio value={0}>普通规则</Radio>
									</Radio.Group>)}
								</FormItem>
							</div>
							:
