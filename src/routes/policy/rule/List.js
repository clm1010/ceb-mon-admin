...
import { Table, Modal, Row, Col, Button, Tooltip, message } from 'antd'
...
function list({
	...
	const handleMenuClick = (record, e) => {
		...
		if (e.key === '5') {
			if (record.ruleType === 'DISTRIBUTE') {
				...
				dispatch({
					type: 'policyRule/offline',
					payload: newitem,
				})
			} else {
				message.info('请选择分布式类型')
			}
		}
	}
	...
	const columns = [
		...
		},
		{
			...
			fixed: 'right',
			render: (text, record) => {
				return <DropOption onMenuClick={e => handleMenuClick(record, e)} menuOptions={[{ key: '1', name: '查看' }, { key: '2', name: '编辑' }, { key: '3', name: '克隆' }, { key: '5', name: '下线' }, { key: '4', name: '删除' }]} />
			},
		},
	]
