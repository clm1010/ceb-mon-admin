...
class Filter extends React.Component {
	constructor(props) {
		...
		this.state.extorFun = props.extorFun
	}
	...
	transformNormal(field, key, getFieldDecorator) {
		switch (field.dataType) {
			case 'int':
				return <Col xl={{ span: 6 }} md={{ span: 6 }} sm={{ span: 24 }} key={key}><FormItem {...formItemLayout} label={field.title}>{getFieldDecorator(field.key, { initialValue: field.defaultValue })(<InputNumber size="default" style={{ width: '100%' }} max={field.max} min={field.min} placeholder={field.placeholder} />)}</FormItem></Col>
	...
	transformBetween(field, key, getFieldDecorator) {
		switch (field.dataType) {
			...
			case 'datetime':
				...
				return <Col xl={{ span: 6 }} md={{ span: 6 }} sm={{ span: 24 }} key={key} id={`date_time_area_${key}`}>
				...
				<Row gutter={4} style={{ marginTop: 8, marginBottom: 20 }}>
					<Col xl={{ span: 24 }} md={{ span: 24 }} sm={{ span: 24 }}>
						...
						<span style={{ float: 'right', marginTop: 8 }}>
							...
							<Button  style={{ marginLeft: 8 }} onClick={extorFun} >
								列设置
							</Button>
