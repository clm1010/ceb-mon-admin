...
const oelField = JSON.parse(localStorage.getItem('dict')).OelColumns
...
let SelectoelField = []
for (let column of oelField) {
	SelectoelField.push(<Select.Option key={column.key} value={column.key}>{column.name}</Select.Option>)
}
...
const ConditionBasicMode = ({
	...
	const add = (index) => {
		...
		*/
		...
		console.log('add arrs : ', arrs, 'filterIndex : ', newIndexList)
		/*
			修改过滤条件的集合
		*/
		dispatch({
	...
	const remove = (myindex) => {
		...
		console.log('remove : ', myindex, 'arrs ：', arrs, 'indexList : ', indexList)
	...
	const onChange = (val) => {
		...
		*/
		...
		console.log('onChange : ', val, valObj, arrs)
		let maxValue = 0
		for (let val of filterIndex) {
			maxValue = (maxValue < val ? val : maxValue)
		}
