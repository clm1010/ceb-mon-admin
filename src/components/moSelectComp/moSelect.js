...
export default {
  ...
  effects: {
  	* query ({ payload }, { put, call, select }) {
	    ...
	    //如果有外部附加查询条件
	    const externalFilter = yield select(({ moSelect }) => moSelect.externalFilter)
	    if (externalFilter.length > 0) {
	    	qs = ` ( discoveryIP=='*${payload.inputInfo}*' or name == '*${payload.inputInfo}*') and (${externalFilter})`
	    }
	    newdatas.q = qs
	    //请求数据
	    const data = yield call(queryAllTypeOfMO, newdatas)
  	...
  	* queryInterfs ({ payload }, { put, call }) {
  		const data = yield call(allInterfs, payload)
  		if (data.success) {
  			yield put({
  				type: 'setState',
  				payload: ({
  					sOptions: data.content,
  				}),
  			})
  		}
  	},
