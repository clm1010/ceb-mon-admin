...
function addLine(line, isTextNode) {
  ...
  match = line.match(/<([^\s<>]+) ?([^<>]*)\/>/);
  if (match) {
    let reg = new RegExp("(?<=src=\")[^;]+.(?=\" alt)");
    //let imgMatch = line.match(/(?<=src=")[^;]+.(?=\" alt)/);
    let imgMatch = line.match(reg);
    if (imgMatch) {
      let src = imgMatch[0].replace(/.\/images/,`${window.location.origin}/helpdoc/images`)
 ...
 * @returns {*}
 */
function parseChildren(str,tag) {
  const pattern = /(<[^<>]+>)/g;
  if (!str) {
    return null;
  }
  ...
  if (stack.length > 0) {
    stack.forEach((node) => {
      ...
      if (node.type === NODE_TYPE.text) {
        ...
        if (!currentNode) {
          ...
          if (node.tagName === currentNode.tagName) {
            ...
            if (showFlag && node.text.indexOf('div')>0) count --;
            if (showFlag && count === 0) {
              showFlag= false;
              strInfo = strInfo + ' ' + node.text;
            }
          } else {
            throw new Error('parse error');
          }
        ...
        if (node.text.indexOf(tag)>0) showFlag = true;
        if (showFlag && node.text.indexOf('div')>0) count ++;
      ...
      if (showFlag) {
        strInfo = strInfo + ' ' + node.text;
        //console.log('counter:',count, node.text)
      }
    });
  }
  //console.log(strInfo)
