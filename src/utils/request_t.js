...
//删除了关于url的相关代码,解决性能中地址存在*的情况：  如 u2hourperformance-2022.06.*/_search/的情况
//  try {
//   let domin = ''
//   if (url.match(/[a-zA-z]+:\/\/[^/]*/)) {
...
export default function request(options) {
  ...
  && options.url.indexOf('/fileAssistantAPI/') === -1 && options.url.indexOf('/flinkapi/') === -1) {
    ...
    fbRoute = '';
  }else if (options.url.indexOf('/fileAssistantAPI/')!=-1) {
    axios.defaults.headers.common.Authorization = `Basic dXNlcjpDaGluYUA5MzA5MjA=`
    axios.defaults.headers.common['Content-Type'] = 'multipart/form-data'
    fbRoute = '';
  }else if (options.url.indexOf('/flinkapi/')!=-1) {
    axios.defaults.headers.common.Authorization = `Basic dXNlcjpDaGluYUA5MzA5MjA=`
    fbRoute = '';
  } else {
