...
export default function xykrequest (options) {
  ...
  //如果是登陆的请求，则不加token串
	if (options.url.indexOf('/api/v1/users/login/searchxykalarm') === -1 && options.url.indexOf('/api/v1/users/iam-sso') === -1) {
  ...
  //指明请求头是UTF-8
  if (options.url.indexOf('/api/v1/users/iam-sso') !== -1) {
  ...
  return fetch(options).then((response) => {
    ...
    if (response) {
      ...
      if (data.refreshToken) {
        sessionStorage.setItem('xyktoken', data.refreshToken)
        request(options)
      }
