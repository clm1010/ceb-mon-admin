...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
      	//初次访问
		  if (location.pathname === '/dbService' && location.search.length === 0) {				//@@@
          ...
          dispatch(routerRedux.push({
        ...
        } else if (location.pathname === '/dbService' && location.search.length > 0) {
					//自动触发mo查询
	        ...
	        if (query.secondClass !== undefined && query.secondClass !== '') {
				// console.long(query.secondClass)
	        	dispatch({
	        			type: 'setState',
	        			payload: {
  ...
  effects: {
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ dbService }) => dbService.pagination)
	  ...
	  * update ({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ dbService }) => dbService.currentItem)				//@@@
