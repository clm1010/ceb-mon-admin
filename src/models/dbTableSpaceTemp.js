...
export default {
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            history.listen((location) => {
                //初次访问
                if (location.pathname === '/dbTableSpaceTemp' && location.search.length === 0) {				//@@@
                ...
                } else if (location.pathname === '/dbTableSpaceTemp' && location.search.length > 0) {
                    //自动触发mo查询
    ...
    effects: {
        * query({ payload }, { call, put }) {
            ...
            // const data = yield call(createDbInsts, payload)
            console.log(data)
        ...
        * requery({ payload }, { select, put }) {
            let pageItem = yield select(({ dbTableSpaceTemp }) => dbTableSpaceTemp.pagination)
        ...
        * update({ payload }, { select, call, put }) {
            ...
            let currentItem = {}
            currentItem = yield select(({ dbTableSpaceTemp }) => dbTableSpaceTemp.currentItem)				//@@@
        ...
        * create({ payload }, { select, call, put }) {
            ...
            const dbinstObj = Object.assign({}, currentItem, payload.currentItem)

            const data = yield call(createdbs, dbinstObj)
