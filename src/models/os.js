...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
      	//初次访问
        if (location.pathname === '/os' && location.search.length === 0) {				//@@@
          ...
          dispatch(routerRedux.push({
				...
				} else if (location.pathname === '/os' && location.search.length > 0) {			//@@@
  ...
  effects: {
	* query ({ payload }, { call, put }) {
	  const data = yield call(queryos, payload)				//@@@
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ os }) => os.pagination)
	  ...
	  * update ({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ os }) => os.currentItem)				//@@@

			currentItem = Object.assign(currentItem, payload.currentItem)

			let data = {}
			data = yield call(updateos, currentItem)				//@@@
			let info = []
	  	if (data.success) {
	  ...
	  * create ({ payload }, { select, call, put }) {
	  	...
	  	const data = yield call(createos, currentItem)				//@@@
	  ...
	  * delete ({ payload }, { select, call, put }) {
		 		const data = yield call(deleteos, payload)				//@@@
