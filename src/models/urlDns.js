...
import { queryurlDns, createurlDns, updateurlDns, deleteurlDns, removeurlDns } from '../services/mo/urlDns'
...
export default {
	...
	state: {
		...
		alertMessage: '请输入url信息',										//alert控件内容
		moImportFileList: [],
	...
	subscriptions: {
		setup({ dispatch, history }) {
			history.listen((location) => {
				//初次访问
				if (location.pathname === '/urlDns' && location.search.length === 0) {				//@@@
				...
				} else if (location.pathname === '/urlDns' && location.search.length > 0) {			//@@@
					//自动触发mo查询
	...
	effects: {
		* query({ payload }, { call, put }) {
			const data = yield call(queryurlDns, payload)				//@@@
		...
		* requery({ payload }, { select, put }) {
			let pageItem = yield select(({ urlDns }) => urlDns.pagination)
		...
		* update({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ urlDns }) => urlDns.currentItem)				//@@@
			currentItem = Object.assign(currentItem, payload.currentItem)

			let data = {}
			data = yield call(updateurlDns, currentItem)				//@@@

			if (data.success) {
				message.success('修改成功！')
				payload.currentItem = data
				payload.alertType = 'success'
				payload.alertMessage = '修改成功。'				//@@@
		...
		* create({ payload }, { select, call, put }) {
			...
			const data = yield call(createurlDns, currentItem)				//@@@

			if (data.success) {
				message.success('保存成功！')
				payload.alertType = 'success'
				payload.alertMessage = '保存成功。'				//@@@
				payload.currentItem = {}
		...
		* delete({ payload }, { select, call, put }) {
			const data = yield call(deleteurlDns, payload)				//@@@
		...
		* deleteAll({ payload }, { select, call, put }) {
			let data = {}
			data = yield call(removeurlDns, payload)				//@@@
