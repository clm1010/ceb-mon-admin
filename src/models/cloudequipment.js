...
import { query, remove, update, create, findById } from '../services/mo/clouddevices'
import {findAllApp, query as queryApp} from '../services/appCategories'
import { message ,Select} from 'antd'
import queryString from "query-string";
const Option = Select.Option
export default {
	namespace: 'cloudequipment',
	...
	subscriptions: {
		setup ({ dispatch, history }) {
			history.listen((location) => {
	      	//初次访问
	      		if (location.pathname === '/cloudequipment' && location.search.length === 0) {
				...
				} else if (location.pathname === '/cloudequipment' && location.search.length > 0) {
	...
	effects: {
		...
		* create ({ payload }, { put, call, select }) {
			let currentItem = {}
			currentItem = yield select(({ cloudequipment }) => cloudequipment.currentItem)
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ cloudequipment }) => cloudequipment.pagination)
