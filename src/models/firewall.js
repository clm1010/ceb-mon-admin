...
export default {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			history.listen((location) => {
				//初次访问
				if (location.pathname === '/firewall' && location.search.length === 0) {				//@@@
				...
				} else if (location.pathname === '/firewall' && location.search.length > 0) {			//@@@
					//自动触发mo查询
	...
	effects: {
		...
		* requery({ payload }, { select, put }) {
			let pageItem = yield select(({ firewall }) => firewall.pagination)
		...
		* update({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ firewall }) => firewall.currentItem)				//@@@
		...
		* create({ payload }, { select, call, put }) {
			...
			payload.currentItem.secondClass = 'FIREWALL'				//@@@
