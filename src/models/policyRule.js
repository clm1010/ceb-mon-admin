import { updateMonitorInstance, query, create, remove, update, check, queryTemplets, queryRuleById, queryMonitorInstanceById, calc, issue, CopyTo, MoveTo, status ,onDown,offline} from '../services/policyRule'
...
pane
	...
	//      		  		var num=Math.random();
	      					...
	      					let data22 = {
	    						...
	    						mo: (mos[0].mo ? mos[0].mo.name : ''),
	     						...
	     						tool: mos[0].tool,
	     						type,
	      						tname: (mos[0].policy.isStd ? mos[0].policy.name : mos[0].policy.name),
	...
	//								var num1=Math.random();
										...
										kpi:	(mos[index].policy && mos[index].policy.monitorParams && mos[index].policy.monitorParams.indicator ? mos[index].policy.monitorParams.indicator.name : ''),
	     					  			tool: mos[index].tool,
	     					  			type: type1,
	      								tname: (mos[index].policy.isStd ? mos[index].policy.name : mos[index].policy.name),
    	...
    	* requery ({ payload }, { put, select }) {
    		let pageItem = yield select(({policyRule}) => policyRule.pagination)
		...
		*offline({ payload }, { call, put }) {
			message.loading('正在下下线,请稍后...', 0)
			const data = yield call(offline, payload)
			if(data.success){
				message.destroy()
				message.success('下线完成！')
			} else {
				message.destroy()
				message.error(data.msg)
			}
