...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
		  //初次访问
		  if (location.pathname === '/dbDataBase' && location.search.length === 0) {				//@@@
        ...
        } else if (location.pathname === '/dbDataBase' && location.search.length > 0) {
					//自动触发mo查询
  ...
  effects: {
	* query ({ payload }, { call, put }) {
	...
	// const data = yield call(createdbDataBases, payload)
	  console.log(data)
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ dbDataBase }) => dbDataBase.pagination)
	  ...
	  * update ({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ dbDataBase }) => dbDataBase.currentItem)				//@@@
	  ...
	  * create ({ payload }, { select, call, put }) {
	  	...
	  	//取currentItem是为了获取完整的对象来全量update后端的mo对象
			...
			const  dbDataBaseObj = Object.assign({}, currentItem, payload.currentItem)

	  	const data = yield call(createdbs, dbDataBaseObj)
