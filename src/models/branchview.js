...
import { branchCfgs } from '../utils/charts'
...
export default {
  ...
  subscriptions: {
      ...
      history.listen((location) => {
        if (location.pathname.includes('/branchview')) {
          dispatch({
            type: 'query',
			...
			for (let chart of branchCfgs) {
				...
				if (chart.oelFilter === '' || chart.oelFilter === undefined) {
					...
					if ('filter' in filter) {
						...
						filterStr = filterStr.replace(/\-/g, '%2D')
						...
						chart.whereSQL = ` (${filterStr})${preFilter}`
