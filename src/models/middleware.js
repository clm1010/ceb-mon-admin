...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
      	//初次访问
        if (location.pathname === '/middleware' && location.search.length === 0) {				//@@@
          //制造一个浏览器地址栏看得见的跳转，仅仅是跳转，再次触发subscription监听进行query查询
		  const { pathname } = location
		  let qq= location.query?   location.query.q : 'firstClass==\'MW\'';
        ...
        } else if (location.pathname === '/middleware' && location.search.length > 0) {			//@@@
					//自动触发mo查询
  ...
  effects: {
	  ...
	  const data = yield call(querymws, payload)				//@@@
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ middleware }) => middleware.pagination)
	  ...
	  * update ({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ middleware }) => middleware.currentItem)				//@@@
			currentItem = Object.assign(currentItem, payload.currentItem)

			let data = {}
			data = yield call(updatemws, currentItem)				//@@@

	  	if (data.success) {
	  		message.success('设备修改成功！')
	  		payload.currentItem = data
	  		payload.alertType = 'success'
	    	payload.alertMessage = '中间件修改成功。'				//@@@
	  ...
	  * create ({ payload }, { select, call, put }) {
			payload.currentItem.firstClass = 'MW'
			let currentItem = {}
			currentItem = yield select(({ middleware }) => middleware.currentItem)				//@@@
			currentItem = Object.assign(currentItem, payload.currentItem)			//@@@
	  	...
	  	const data = yield call(createmws, currentItem)				//@@@

	    if (data.success) {
	    	message.success('设备保存成功！')
	    	payload.alertType = 'success'
	    	payload.alertMessage = '中间件保存成功。'				//@@@
	    	payload.currentItem = {}
	    	payload.modalVisible = false
	  ...
	  * delete ({ payload }, { select, call, put }) {
		 		const data = yield call(deletemws, payload)				//@@@
