...
export default {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			history.listen((location) => {
				//初次访问
				if (location.pathname === '/branchIp' && location.search.length === 0) {				//@@@
				...
				} else if (location.pathname === '/branchIp' && location.search.length > 0) {			//@@@
					//自动触发mo查询
	...
	effects: {
		* query({ payload }, { call, put }) {
			const data = yield call(querybranchips, payload)				//@@@
		...
		* requery({ payload }, { select, put }) {
			let pageItem = yield select(({ branchIp }) => branchIp.pagination)
		...
		* update({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ branchIp }) => branchIp.currentItem)				//@@@
		...
		* create({ payload }, { select, call, put }) {
			...
			payload.currentItem.secondClass = 'BRANCH_IP'				//@@@
			payload.currentItem.keyword = payload.currentItem.discoveryIP

			//state里的item和表单提取的item合并
			const branchIpObj = Object.assign({}, currentItem, payload.currentItem)				//@@@

			const data = yield call(createbranchips, branchIpObj)				//@@@

			if (data.success) {
				message.success('设备保存成功！')
				payload.alertType = 'success'
				payload.alertMessage = '网点IP保存成功。'				//@@@
				payload.currentItem = {}
		...
		* delete({ payload }, { select, call, put }) {
			const data = yield call(deletebranchips, payload)				//@@@
