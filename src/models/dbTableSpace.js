...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
      	//初次访问
		  if (location.pathname === '/dbTableSpace' && location.search.length === 0) {				//@@@
        ...
        } else if (location.pathname === '/dbTableSpace' && location.search.length > 0) {
					//自动触发mo查询
  ...
  effects: {
	* query ({ payload }, { call, put }) {
	...
	// const data = yield call(createDbTableSpaces, payload)
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ dbTableSpace }) => dbTableSpace.pagination)
	  ...
	  * update ({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ dbTableSpace }) => dbTableSpace.currentItem)				//@@@
	  ...
	  * create ({ payload }, { select, call, put }) {
	  	...
	  	const data = yield call(createdbs, currentItem)
	    ...
	    if (data.success) {
	    	...
	    	payload.currentItem = data
	    	payload.modalVisible = false
			// const testData = yield call(createDbInsts,currentItem );
