...
export default {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			history.listen((location) => {
				//初次访问
				if (location.pathname === '/f5' && location.search.length === 0) {				//@@@
				...
				} else if (location.pathname === '/f5' && location.search.length > 0) {			//@@@
					//自动触发mo查询
	...
	effects: {
		...
		* requery({ payload }, { select, put }) {
			let pageItem = yield select(({ f5 }) => f5.pagination)
		...
		* update({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ f5 }) => f5.currentItem)				//@@@
		...
		* create({ payload }, { select, call, put }) {
			...
			payload.currentItem.secondClass = 'F5'				//@@@
