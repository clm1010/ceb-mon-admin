...
import { queryOsDisk, createOsDisk, updateOsDisk, deleteOsDisk,removeOsDisk } from '../services/mo/osDisk'
...
export default {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			history.listen((location) => {
				//初次访问
				if (location.pathname === '/osDisk' && location.search.length === 0) {				//@@@
					//制造一个浏览器地址栏看得见的跳转，仅仅是跳转，再次触发subscription监听进行query查询
					const { pathname } = locationDISK
					const query = {
				...
				} else if (location.pathname === '/osDisk' && location.search.length > 0) {
					//自动触发mo查询
	...
	effects: {
		* query({ payload }, { call, put }) {
			const data = yield call(queryOsDisk, payload)
		...
		* requery({ payload }, { select, put }) {
			let pageItem = yield select(({ osDisk }) => osDisk.pagination)
		...
		* update({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ osDisk }) => osDisk.currentItem)				//@@@
			currentItem = Object.assign(currentItem, payload.currentItem)
			let data = {}
			data = yield call(updateOsDisk, currentItem)
			if (data.success) {
		...
		* create({ payload }, { select, call, put }) {
			...
			const osDiskObj = Object.assign({}, currentItem, payload.currentItem)
			const data = yield call(createOsDisk, osDiskObj)
			if (data.success) {
		...
		* delete({ payload }, { select, call, put }) {
			const data = yield call(deleteOsDisk, payload)
			if (data.success) {
		...
		* deleteAll({ payload }, { select, call, put }) {
			let data = {}
			data = yield call(removeOsDisk, payload)
			if (data.success) {
				message.success('设备删除成功！')
				yield put({ type: 'requery' })
			} else {
