...
import { query, remove, update, create, findById } from '../services/mo/hardwareServer'
...
export default {
	...
	subscriptions: {//hardwareServer
		setup ({ dispatch, history }) {
			history.listen((location) => {
	      	//初次访问
		      	if (location.pathname === '/hardwareServer' && location.search.length === 0) {
				...
				} else if (location.pathname === '/hardwareServer' && location.search.length > 0) {
	...
	effects: {
		...
		* create ({ payload }, { put, call, select }) {
			let currentItem = {}
			currentItem = yield select(({ hardwareServer }) => hardwareServer.currentItem)
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ hardwareServer }) => hardwareServer.pagination)
