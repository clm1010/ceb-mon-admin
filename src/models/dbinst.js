...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
		  //初次访问
		  if (location.pathname === '/dbinst' && location.search.length === 0) {				//@@@
        ...
        } else if (location.pathname === '/dbinst' && location.search.length > 0) {
					//自动触发mo查询
  ...
  effects: {
	...
	* requery({ payload }, { select, put }) {
		let pageItem = yield select(({ dbinst }) => dbinst.pagination)
	  ...
	  * update ({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ dbinst }) => dbinst.currentItem)				//@@@
