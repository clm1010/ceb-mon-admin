import { query, sql, alarmProcessServ, getJournal, knowledges, getAlarmTree, getAlarmCompact,queryRecommend, outCall, outCalluser, queryHint } from '../services/alarms'
...
export default {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			history.listen((location) => {
				if (location.pathname.includes('/oelCompression')) {
	...
	effects: {
		...
		* getJournal({ payload }, { call, put, select }) {
			let branchType = yield select(state => state.oelcompression.branchType)
			// 从state中找出数据源id和osid
			const oelDatasource = yield select(state => state.oelcompression.oelDatasource)
			const osUuid = yield select(state => state.oelcompression.osUuid)
		...
		* sqlTrigger({ payload }, { call, put, select }) {
			...
			let pagination = yield select(state => state.oelcompression.pagination)
			let oelFilter = yield select(state => state.oelcompression.oelFilter)
			let oelViewer = yield select(state => state.oelcompression.oelViewer)
			let oelDatasource = yield select(state => state.oelcompression.oelDatasource)
			let osUuid = yield select(state => state.oelcompression.osUuid)
		...
		* query({ payload }, { call, put, select }) {
			...
			//取得非字符串类型字段名集合备用
			let nonStringFields = yield select(state => state.oelcompression.nonStringFields)
			...
			//取得日期类型字段名集合备用
			let dateFields = yield select(state => state.oelcompression.dateFields)
			...
			//生成过滤器下拉列表
			const filterList = yield select(state => state.oelcompression.filterList)
			...
			//过滤器
			const oelFilterState = yield select(state => state.oelcompression.oelFilter)
			//过滤器字符串
			const qFilter = yield select(state => state.oelcompression.qFilter)
			...
			const tagFilters = yield select(state => state.oelcompression.tagFilters)
			let qTagFilter = ''
			...
			if (payload.orderBy !== '' && payload.orderBy !== undefined) {
				...
				const orderBy = yield select(state => state.oelcompression.orderBy)
				if (orderBy.length > 0) {
					qOrderBy = orderBy
				}
			}
			...
			if (!('preFilter' in payload)) {
				preFilter = yield select(state => state.oelcompression.preFilter)
			} else {
			...
			// 树节点传入参数
			let treeFilter = ''
			let saveSelected = yield select(state => state.oelcompression.treeSelected)
			let treeSelected = undefined

			//console.log('choose treenode:',payload.treeSelected)
			if (('treeSelected' in payload) && (JSON.stringify(payload.treeSelected) !== "{}")) { // select tree nodes
				treeSelected = payload.treeSelected
			} else if ((JSON.stringify(payload.treeSelected) == "{}") && !('selectedKeys' in payload)) { // auto refresh
				treeSelected = saveSelected
				payload.treeSelected = saveSelected
			} // for unselected node, payload.treeSelected == {} && payload.selectedKeys == [], np need to reset it
			if (treeSelected) {
			...
			if (!('treeSelected' in payload)) { //clear tree's selectedKeys for changing query conditions
				payload.treeSelected = {}
				payload.selectedKeys = []
			} //else if (JSON.stringify(payload.treeSelected) == "{}") {}//console.log('got here333:',saveSelected) //auto refresh
			...
			const datasourceList = yield select(state => state.oelcompression.datasourceList)
			...
			//视图列表
			const viewList = yield select(state => state.oelcompression.viewList)
			const oelViewer = yield select(state => state.oelcompression.oelViewer)
			const oelColumns = yield select(state => state.oelcompression.oelColumns)
			...
			//工具集合列表
			const toolList = yield select(state => state.oelcompression.toolList)
			...
			*/
			...
			// 判断是否查询压缩
			let data = {}
			const isPackedAlarms = ('isPackedAlarms' in payload) ? payload.isPackedAlarms : yield select(state => state.oelcompression.isPackedAlarms)
			if (isPackedAlarms) {
				data = yield call(getAlarmCompact, payload)
			} else {
				data = branchType == 'XYK' ? yield call(xykquery, payload) : yield call(query, payload)
			}
			if (data.success) {
		...
		* querySubAlarms({ payload }, { call, put, select }) {
			let branchType = yield select(state => state.oelcompression.branchType)
			//如果传过来的ObjectServer的Uuid不存在，则默认使用state里存储的uuid
			payload.osUuid = yield select(state => state.oelcompression.osUuid)
			...
			//分页数据
			let pagination = yield select(state => state.oelcompression.subPagination)
			if (payload.page !== undefined) {
				pagination.current = payload.page
			} else {
				pagination.current = pagination.current
			...
			let alarmSelected = payload.alarmSelected ? payload.alarmSelected : yield select(state => state.oelcompression.alarmSelected)
			...
			//添加树节点查询条件
			let treeSelected = yield select(state => state.oelcompression.treeSelected)
			let treeFilter = ''
