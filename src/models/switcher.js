...
export default {
	...
	subscriptions: {
		setup({ dispatch, history }) {
			history.listen((location) => {
				//初次访问
				if (location.pathname === '/switcher' && location.search.length === 0) {				//@@@
				...
				} else if (location.pathname === '/switcher' && location.search.length > 0) {			//@@@
					//自动触发mo查询
	...
	effects: {
		...
		* requery({ payload }, { select, put }) {
			let pageItem = yield select(({ switcher }) => switcher.pagination)
		...
		* update({ payload }, { select, call, put }) {
			...
			let currentItem = {}
			currentItem = yield select(({ switcher }) => switcher.currentItem)				//@@@
		...
		* create({ payload }, { select, call, put }) {
			...
			payload.currentItem.secondClass = 'SWITCH'				//@@@
