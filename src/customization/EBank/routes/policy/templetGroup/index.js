 ...
 *
 * 此页面用于添加、修改和删除模板组和模板。
 * 页面分两部分：分组树（左）和模板列表（右）。选择左侧的分组将出现不同的模板列表。
 * 
 * ## 分组操作
 * ##### 添加分组
 * 添加新的分组，点击弹出新增模板组窗口。
...
function policyTempletGroup ({
 ...
 treeDatas, selectTreeNode, selectKeys, modalVisible, modalType, isClose,
	...
	const onSelect = (selectedKeys, info) => {
		...
		dispatch(routerRedux.push(`/policyTempletGroup/policyTemplet`)) //显示第三级页面
	}
	...
	const onDelete = () => {
		...
		confirm({
			...
			onOk () {
			  dispatch({
					type: 'policyTempletGroup/delete',
  ...
  <Row gutter={12} className="content-tree">
    <Col lg={expand ? 5 : 0} md={expand ? 6 : 0} sm={expand ? 7 : 0} xs={24} style={{ padding: 0 }}>
      <div style={policyTemplet.heightSet}>
 ...
 width: '100%', height: '90%', float: 'left', overflow: 'scroll',
    ...
    <Col lg={expand ? 19 : 24} md={expand ? 18 : 24} sm={expand ? 17 : 24} xs={24} className="content-right">
      <div className="lines" />
      <Switch>
        <Route path='/policyTempletGroup/policyTemplet' render={() => (<Templet {...tempLetProps}/>)}/>
      </Switch>
    </Col>
  </Row>
	)
...
policyTempletGroup.propTypes = {
  policyTempletGroup: PropTypes.object,
}

//export default connect()(StdIndicators)
export default connect(({ policyTempletGroup, loading, policyTemplet, stdIndicatorGroup }) => ({ policyTempletGroup, policyTemplet,stdIndicatorGroup, loading: loading.models.policyTempletGroup }))(policyTempletGroup)
