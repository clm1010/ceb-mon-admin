 ...
 * @module 监控配置/监控实例管理 
 * @description 
 * URL: <u>/ruleInstanceGroup/ruleInstance</u>
 *
 * 此页面用于添加、修改和删除实例组及实例。
...
import { Row, Col, Modal, Tree } from 'antd'
...
function ruleInstanceGroup ({
 children, location, dispatch, ruleInstanceGroup, loading, ruleInstance,objectMOsModal,stdIndicatorGroup
}) {
	const { expand } = ruleInstance
	const {
 treeDatas, selectTreeNode, selectKeys, modalVisible, modalType, isClose,
} = ruleInstanceGroup
  const ruleinstanceProps = {
    ruleInstanceGroup,
    location,
    dispatch,
    objectMOsModal,
	...
	const onSelect = (selectedKeys, info) => {
		...
		dispatch({ //把选择中的 key 保存到state中，以便在其他触发事件中使用
			type: 'ruleInstanceGroup/controllerState',
			payload: {
		...
		dispatch({ //把选择中的 key 保存到指标信息的state中，以便查询时，获取对应的指标
			...
			payload: {
				groupUUIDs: selectedKeys,
				filterKey: `${new Date().getTime()}`,
			},
		})
		//dispatch(routerRedux.push(`/ruleInstanceGroups`)) //显示第三级页面
		dispatch(routerRedux.push(`/ruleInstanceGroup`)) //显示第三级页面
	}
  ...
  <Row gutter={12} className="content-tree">
    <Col lg={expand ? 5 : 0} md={expand ? 6 : 0} sm={expand ? 7 : 0} xs={24} style={{ padding: 0 }}>
      <div style={ruleInstance.heightSet}>
        <div style={{ margin: '12px 6px' }}>实例组
          <span>
        ...
        <div style={{
 width: '100%', height: '100%', float: 'left', overflow: 'scroll',
}}
        >
          {/*
					<Tree
    ...
    <Col lg={expand ? 19 : 24} md={expand ? 18 : 24} sm={expand ? 17 : 24} xs={24} className="content-right">
      <div className="lines" />
      <Switch>
        <Route path='/ruleInstanceGroup/ruleInstance' render={() => (<RuleInstance {...ruleinstanceProps}/>)}/>
      </Switch>
    </Col>
  </Row>
	)
...
ruleInstanceGroup.propTypes = {
  ruleInstanceGroup: PropTypes.object,
}

//export default connect()(StdIndicators)
export default connect(({ ruleInstanceGroup, loading, ruleInstance, objectMOsModal, stdIndicatorGroup }) => ({ ruleInstanceGroup, ruleInstance, objectMOsModal, stdIndicatorGroup, loading: loading.models.ruleInstanceGroup }))(ruleInstanceGroup)
