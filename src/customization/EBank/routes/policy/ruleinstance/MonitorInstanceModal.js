...
const modal = ({
	...
	const onOk = () => {
		validateFieldsAndScroll((errors) => {
			...
			//saveitem.policy.name = 'test88888888888888'
			...
			if (type === 'create') {
				dispatch({
					type: 'ruleInstance/createMonitorInstance',											//抛一个事件给监听这个type的监听器
					payload: saveitem,
				})
			} else {
				dispatch({
					type: 'ruleInstance/updateMonitorInstance',											//抛一个事件给监听这个type的监听器
					payload: saveitem,
				})
			}
			resetFields()
...
*/
	...
	*/
	const onobjectInfo = () => {
		...
		dispatch({
			type: 'ruleInstance/showModal', //抛一个事件给监听这个type的监听器
			payload: {
	...
	return (
		...
		>
			<Form layout="horizontal">
				<Tabs defaultActiveKey="ruleInstance_1">
					<TabPane tab={<span><Icon type="user" />基本信息</span>} key="ruleInstance_1">
				...
				<Tabs defaultActiveKey="ruleInstance_2">
					<TabPane tab={<span><Icon type="appstore" />监控工具</span>} key="ruleInstance_2">
				...
				<Tabs defaultActiveKey="ruleInstance_3">
					<TabPane tab={<span><Icon type="global" />监控对象</span>} key="ruleInstance_3">
				...
				</Tabs>
				{
					((typeValue !== 'SYSLOG')) ?
						<div>
							<Tabs defaultActiveKey="ruleInstance_4">
								<TabPane tab={<span><Icon type="setting" />监控参数</span>} key="ruleInstance_4">
