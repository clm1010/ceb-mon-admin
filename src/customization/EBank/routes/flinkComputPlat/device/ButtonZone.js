...
const buttonZone = ({ dispatch, batchDelete, choosedRows, showUploadList, moImportFileList, tableStatue, contions }) => {
	...
	const onDownOut = () => {
		confirm({
			...
			onOk() {
				dispatch({
					type: 'flinkComputPlat/onDownDevice',
					payload: {
						contions,
						filename: '监控设备',
					}
	...
	const onbindGroup = (type) => {
		dispatch({
			type: 'flinkComputPlat/setState',
			payload: {
				bindgoupVisible: true,
				bindGroupTyep: type
			},
		})
		if (type == 'bind') {
			...
			dispatch({
				type: 'flinkComputPlat/query_Dev_Group',
				payload: {
					id:choosedRows[0]
				},
			})
	...
	return (
		<Row gutter={24} style={{ marginTop: 8, marginBottom: 8 }} >
			<Col lg={24} md={24} sm={24} xs={24}>
				...
				<Button size="default" type="ghost" onClick={onDeviceAdd} style={{ marginLeft: 10 }} >设备新增</Button>
				<Button size="default" type="ghost" onClick={onDeviceDel} style={{ marginLeft: 10 }} disabled={!batchDelete}>设备下线</Button>
				<Button size="default" type="ghost" onClick={() => onbindGroup('bind')} style={{ marginLeft: 10 }} disabled={!batchDelete}>绑定模板组</Button>
				<Button size="default" type="ghost" onClick={() => onbindGroup('unbind')} style={{ marginLeft: 10 }} disabled={!batchDelete}>解绑模板组</Button>
