import { osfsquery } from '../../../../../services/mo/os'

export default {

  namespace: 'osSingleSelect',
  ...
  effects: {
  	* query ({ payload }, { put, call, select }) {
	    ...
	    let qs = newdatas.q
	    qs = `firstClass=='OS' and (discoveryIP=='*${payload.inputInfo}*' or name == '*${payload.inputInfo}*')`

	    //如果有外部附加查询条件
	    const externalFilter = yield select(({ osSingleSelect }) => osSingleSelect.externalFilter)
	    ...
	    newdatas.q = qs
		//请求数据
	    const data = yield call(osfsquery, newdatas)
	    if (data.success) {
	    	yield put({
	    		type: 'setState',
	    		payload: {//必须使用
