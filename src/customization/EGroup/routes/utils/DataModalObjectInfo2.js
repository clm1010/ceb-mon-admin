 ...
 treeDatas, treeMap, firstClass, secondClass, thirdClass, selectuuid, pagination, list,
	...
	*/
	...
	const onOk = () => {
	    ...
	    if (selectType === 'jia') {
	    	for (let i = 0; i < jiaList.length; i++) {
	    		let mo = jiaList[i]
	    		if (mo.index === selectIndex) {
	    			mo.moname = selectItemObj[0].name
	    			mo.mouuid = selectItemObj[0].uuid
	    		}
	    	}
	    } else {
