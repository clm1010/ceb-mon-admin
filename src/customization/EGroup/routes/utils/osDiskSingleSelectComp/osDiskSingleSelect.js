import { queryOs } from '../../../../../services/mo/osDisk'

export default {

  namespace: 'osDiskSingleSelect',

  state: {
    disabled: false, 					 //控制搜索的可用性
    placeholders: '请输入信息',		//文本框里的提示信息
    options: [],								//选择搜索框的内容
    name: '所属操作系统',						 //组件的名字 要加入
  ...
  effects: {
  	* query ({ payload }, { put, call, select }) {
	    ...
	    let qs = newdatas.q
	    qs = `firstClass=='OS' and (keyword=='*${payload.inputInfo}*' or name == '*${payload.inputInfo}*')`
		 
	    //如果有外部附加查询条件
	    const externalFilter = yield select(({ osDiskSingleSelect }) => osDiskSingleSelect.externalFilter)
	    ...
	    newdatas.q = qs
	    //请求数据
		const data = yield call(queryOs, newdatas)
