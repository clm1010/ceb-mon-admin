			...
			for (let col of cols) {
				if (dateFields.indexOf(col.dataIndex) >= 0) {
					...
					col.render = (text, record) => {
						...
						return <ContextMenuTrigger collect={collect} upperName={col.key} name={col.dataIndex} id="simple" OZ_AlarmID={record.OZ_AlarmID} record={record} holdToDisplay={1000}><div title={record[col.dataIndex]} className="oel-ellipsis" style={{ background: backgroundColor, color }}>{n_CustomerSeverity}</div></ContextMenuTrigger>
			    	}
