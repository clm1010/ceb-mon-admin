...
function patch () {
  let routItem
  let backRoute = JSON.parse(sessionStorage.getItem('data'))
  const click = (field) => {
    ...
    return val.map((item) => {
      ...
      if (item.code) {
        if (item.index === backRoute.length) {
          return <Breadcrumb.Item> <span style={{ color: '#e01083' }}> {item.name}</span></Breadcrumb.Item>
        }
          return (<Breadcrumb.Item> <Link to={`/${data}?q= monitoringTree.parentCode ==${item.code}`}
            style={{ color: '#e01083' }}
            onClick={() => click(item)}
          > {item.name}
          </Link>
