	...
	subscriptions: {
    	setup ({ dispatch, history }) {
	      	history.listen((location) => {
	        	if (location.pathname === '/dashboard/bizarea') {
	...
	effects:{
		*query ({ payload }, { call, put }) {
			...
			let ranges = { range: { clock: { gt: Date.parse(new Date()) / 1000 - 7200 } } } //时间限制
      		...
      		firewallTable.query.filtered.filter.bool.must = []
      		firewallTable.query.filtered.filter.bool.must.push(componetid)
      		firewallTable.query.filtered.filter.bool.must.push(ranges)
      		firewallTable.query.filtered.filter.bool.must.push(branchs)
      		if(payload.firewallName !== "" && payload.firewallName){
      			firewallTable.query.filtered.filter.bool.must.push(payload.firewallName)
      		}
      		if(payload.bizareaName !== "" && payload.bizareaName){
      			firewallTable.query.filtered.filter.bool.must.push(payload.bizareaName)
      		}
      		if(payload.hostipName !== "" && payload.hostipName){
      			firewallTable.query.filtered.filter.bool.must.push(payload.hostipName)
      		}
      		const data = yield call ( queryByDay ,firewallTable )
      		let info = data.aggregations.group_bizarea.buckets
      		console.log('聚合结果：',data.aggregations.group_bizarea.buckets)
      		if(info){ 
