...
export default {
	...
	effects: {
		* query ({ payload }, { call, put, select }) {
			...
			if (type === 'cpu') {
				newData.q = `n_ComponentTypeID == 'NetWork';( n_SubComponentID == 'CPU' or  n_SubComponent == 'cpu' );${firstOccurrence}`
			} else if (type === 'men') {
				newData.q = `n_ComponentTypeID == 'NetWork';( n_SubComponentID == 'Memory' or  n_SubComponent == '内存' );${firstOccurrence}`
			} else if (type === 'equDown') {
				newData.q = `n_ComponentTypeID == 'NetWork';agent=='*Ping*';((manager=='*Zabbix*';alertGroup=='ICMP_Failed') or manager!='*Zabbix*');n_ClearTime==1970-01-01T08:00:00;severity!='0';${firstOccurrence}`
			} else if (type === 'equUp') {
				newData.q = `n_ComponentTypeID == 'NetWork';agent=='*Ping*';n_ClearTime>1970-01-01T08:00:00;severity=='0';${firstOccurrence}`
			} else if (type === 'portDown') {
				newData.q = `n_ComponentTypeID == 'NetWork';manager!='*Zabbix*';n_InstanceID!=null;(evenTid=='*DOWN*' or evenTid=='*ADJCHG*');alertGroup!='未知SYSLOG事件';(evenTid!='*_full*' or evenTid!='*_up*');${firstOccurrence}`
			} else if (type === 'portUp') {
				newData.q = `n_ComponentTypeID == 'NetWork';manager!='*Zabbix*';n_InstanceID!=null;(evenTid=='*DOWN*' or evenTid=='*ADJCHG*');alertGroup!='未知SYSLOG事件';(evenTid=='*_full*' or evenTid=='*_up*');${firstOccurrence}`
			}
